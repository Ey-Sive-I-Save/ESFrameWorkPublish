...
| `New` | mode, project root, terminal mode/window/tab title, task/responsibility key, launch token/envelope path, `terminalStarted`, `promptObserved`, `contextAccepted`, startup timeout/failure, and exact acceptance receipt or diagnostic path. |
| `Resume`, `Fork` | exact SessionId or explicit resolution evidence, mode, restored responsibility/title, new launch token/envelope, acceptance state, and any stale-context or source-drift warning. |
| Handoff `New` / acceptance | immutable envelope path, private snapshot directory, handoff file count and validation result, exact launch token, acceptance receipt, source-drift status, and timeline coverage only when a history archive is part of the operation. |
