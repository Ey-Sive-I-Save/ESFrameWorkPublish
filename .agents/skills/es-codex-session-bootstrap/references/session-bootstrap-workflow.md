...
23a. Interpret “只读恢复/读取旧对话后开新窗口，不要 Resume” as `New-ESCodexReadOnlyContext.ps1`: read one exact transcript or semantic archive read-only, create a redacted private context packet, then invoke `Start-ESCodexSession.ps1 -Mode New`. The packet is navigation-only and excludes old SessionId, RecordId, LaunchToken, window/process identity, source absolute paths, and authorization. The resulting envelope records `readOnlyContext=true`, `resumeUsed=false`, and `crossAiResume=false`; this operation never calls Resume/Fork and is not formal Handoff.
23b. Interpret “当前页签清掉后重建/当前页签自交接/会话再生” as `Invoke-ESCodexCurrentTabRecycle.ps1`. It resolves the exact current session from `ES_CODEX_LAUNCH_TOKEN`, creates a read-only packet, opens a receiving tab in the same Windows Terminal window, waits for the receiving acceptance receipt, and only then asynchronously closes the source tab. The source tab remains open on timeout or acceptance failure. This is the frozen `same-window-new-tab-then-close-source` behavior (`physicalTabReused=false`, `samePhysicalTab=false`), not in-place reuse of the old Codex identity; the current caller must explicitly choose `TerminalMode=CurrentWindow` and the operation must never close an unrelated tab. Do not route or report this operation as physical same-tab rebuild.

23c. 对“一次请求启动多个职责窗口/多启动”使用 `Invoke-ESCodexMultiLaunch.ps1 -PlanPath <显式JSON计划>`。计划中的每个 `launches[]` 必须有唯一 `taskKey`、`responsibilityKey` 和 `taskPrompt`，并显式声明 `mode=New|Handoff|Reissue`；批次只做有界编排，不把一个窗口的验收冒充全批次成功。每项启动前查询权威 Session 注册表；同一任务/职责已有非 Closed 会话时返回 `AlreadyRunning`，不得重复开窗。`Handoff` 若携带 `existingEnvelopePath`/`existingLaunchToken`，先以 `-StrictGit` 验证；首次非零或 HEAD 漂移返回 `NeedsReissue`，不得静默切换为 New。`Reissue` 必须由调用者显式加 `-Reissue`，重新通过 `Complete-ESCodexHandoff.ps1` 生成当前 HEAD 的私有快照。输出包含 `batchId`、批次指纹、每项状态、失败原因、`partialFailure` 与 `maxParallel`；超过并发预算只报告分波次，不自动突破宿主并发。默认 DryRun/预检，不宣称 contextAccepted、Runtime 或任务完成。
