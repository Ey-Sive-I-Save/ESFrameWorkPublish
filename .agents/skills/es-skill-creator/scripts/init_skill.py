...
# This project Skill is intentionally scoped to the repository's discoverable
# Skill surface.  Resolving before checking prevents ``..`` and symlinked
# paths from expanding the write target outside the governed tree.
PROJECT_ROOT = Path(__file__).resolve().parents[4]
SKILLS_ROOT = (PROJECT_ROOT / ".agents" / "skills").resolve()
...
def init_skill(skill_name, path, resources, include_examples, interface_overrides):
    ...
    """
    # Determine and constrain the skill directory path.  A creator running in
    # this project must never create a Skill in an external or hidden tree.
    output_root = Path(path).resolve()
    if output_root != SKILLS_ROOT:
        print(f"[ERROR] Output path must be the direct Skill root {SKILLS_ROOT}")
        return None
    skill_dir = (output_root / skill_name).resolve()
    try:
        skill_dir.relative_to(SKILLS_ROOT)
    except ValueError:
        print(f"[ERROR] Skill path must remain inside {SKILLS_ROOT}")
        return None
    ...
    print("4. Run quick_validate.py --require-skill-disclosure when ready")
    print(
