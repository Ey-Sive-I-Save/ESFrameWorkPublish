...
本文件是 `es-skill-governance` 对所有 Project Skill 的统一运行性能约束。它约束执行方式和证据，不扩大任何读、写、Unity、Git、发布、删除或外部网络权限。Skill 的具体业务规则、AIWarnings P0、AICommand 和 TaskContract 仍然优先。

## Execution modes

每次 Skill 运行必须在计划或执行记录中标明一种模式：

- **Fast Path（默认）**：只做命中路由所需的最小读取、P0/权限/TaskContract 校验、输入边界检查和必要的局部验证。
...
- Skill、Catalog、AIWarnings 和治理 Hash 应按稳定身份缓存；来源路径、大小、修改时间或内容 Hash 变化时使对应缓存失效。缓存只能减少未变化文件的重复发现/读取，不能替代本次授权、稳定身份、PlanHash 或必需证据校验。
- 缓存命中不能覆盖权限、身份、PlanHash 或证据失效；失效时重新规划或进入 Deep Path，不能静默使用旧结果。
