...
foreach($entry in @($registry.skills.PSObject.Properties)){
    ...
    if(Test-Path -LiteralPath $govPath -PathType Leaf){try{$gov=Get-Content -Raw -Encoding UTF8 $govPath|ConvertFrom-Json}catch{}}
    ...
    if($hits.Count -gt 0){$matches += [pscustomobject]@{skillName=$skillName;matchedAliases=$hits;routeKeys=@($gov.routeKeys|ForEach-Object {[string]$_});skillPath=('.agents'+'/'+'skills'+'/'+$skillName);skillFileExists=(Test-Path -LiteralPath (Join-Path $skillPath 'SKILL.md') -PathType Leaf)}}
}
