...
$files=@();foreach($sourceRoot in @($manifest.sourceRoots)){try{$full=Resolve-ProjectPath ([string]$sourceRoot);if(-not(Test-Path $full)){[void]$issues.Add("sourceRoot missing: $sourceRoot")}elseif(Test-Path $full -PathType Leaf){$files+=Get-Item $full}else{$files+=Get-ChildItem $full -Recurse -File | Where-Object {$_.FullName -notmatch '[\\/]__pycache__[\\/]' -and $_.Extension -notin @('.pyc','.pyo')}}}catch{[void]$issues.Add($_.Exception.Message)}}
$specializedRegistryRelative='.agents/skills/es-static-deep-replay/references/specialized-acceptance-registry.json'
if(Test-Path -LiteralPath $registryPath -PathType Leaf){$files+=Get-Item -LiteralPath $registryPath}
$evidenceContractRelative='ES/Automation/Contracts/es-skill-evidence-receipt-v1.schema.json'
try{$evidenceContractFull=Resolve-ProjectPath $evidenceContractRelative;if(-not(Test-Path -LiteralPath $evidenceContractFull -PathType Leaf)){[void]$issues.Add('Central Skill Evidence contract is missing')}else{$files+=Get-Item -LiteralPath $evidenceContractFull}}catch{[void]$issues.Add($_.Exception.Message)}
