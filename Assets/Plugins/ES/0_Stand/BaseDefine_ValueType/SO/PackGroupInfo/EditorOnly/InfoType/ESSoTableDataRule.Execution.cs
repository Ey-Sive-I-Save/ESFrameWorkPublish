        ...
        private int ApplyTableRowsToExistingObjects(List<List<string>> table)
        {
            ...
            for (int rowIndex = GetDataStartRowIndex(table); rowIndex < table.Count; rowIndex++)
            {
                ...
                if (serialChildRows)
                {
                    ...
                    {
                        Debug.LogWarning("Row " + (rowIndex + 1) + " skipped: child row Key is empty. Empty child Key requires allowEmptyRowKey and a List row container.", owner);
                        continue;
                    }
                    ...
                    if (directive.directive != ESTableRowDirective.Owner && keylessChildByOrder)
                    {
                        ...
                        {
                            Debug.LogWarning("Row " + (rowIndex + 1) + " keyless child import failed: " + keylessReason, owner);
                            continue;
                        }
                ...
                ApplyTableRowToObject(row, compiledColumns, owner, rowObject, forceTableValues, skipEmptyCells, ownerColumnsOnly, rowDebug, rowIndex);
                EditorUtility.SetDirty(owner);
                changedCount++;
                if (rowDebug != null)
                    Debug.Log(rowDebug.ToString(), owner);
                ...
                {
                    Debug.LogWarning("Row " + (rowIndex + 1) + " import failed and was skipped. Error: " + e.Message, this);
                }
            }
