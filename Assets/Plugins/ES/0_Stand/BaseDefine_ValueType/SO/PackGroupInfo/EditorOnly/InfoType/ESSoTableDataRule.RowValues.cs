...
namespace ES
{
    public partial class ESSoTableDataRule
    {
        ...
        private void ApplyTableRowToObject(List<string> row, List<ESTableCompiledColumn> compiledColumns, ScriptableObject owner, object rowObject, bool forceTableValues, bool skipEmptyCells, bool ownerColumnsOnly, StringBuilder debugLog = null, int rowIndex = -1)
        {
            for (int i = 0; i < compiledColumns.Count; i++)
            {
                ...
                {
                    string rowText = rowIndex >= 0 ? "Row " + (rowIndex + 1) + ", " : string.Empty;
                    string columnName = column.map != null ? GetActiveTableColumnName(column.map) : "(unknown column)";
                    Debug.LogWarning(rowText + "Column " + (column.tableColumnIndex + 1) + " `" + columnName + "` write failed and was skipped. Error: " + e.Message, owner);
