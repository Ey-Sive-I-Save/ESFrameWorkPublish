...
namespace ES
{
    public partial class ESSoTableDataRule
    {
        ...
        public void DebugConfiguredTableRow()
        {
            ...
            if (rowIndex < GetDataStartRowIndex(table) || rowIndex >= table.Count)
            {
        ...
        private void AppendFullImportPlan(StringBuilder builder, ESTablePlanRiskSummary summary)
        {
            ...
            for (int rowIndex = dataStartRow; rowIndex < table.Count; rowIndex++)
            {
                ...
                if (serialChildRows && !inheritedSerialOwner && owner != null)
                ...
                AppendFieldChangePlan(builder, row, compiledColumns, owner, rowObject, directive, summary, rowIndex, inheritedSerialOwner);
                }
                catch (Exception e)
                {
                    summary.skippedRows++;
        ...
        private void ValidateColumnAssertions(List<List<string>> table, List<ESTableCompiledColumn> compiledColumns, ESTableBatchApplyFilter applyFilter, ESTablePlanRiskSummary summary)
        {
            ...
            for (int rowIndex = dataStartRow; rowIndex < table.Count; rowIndex++)
            {
                ...
                for (int i = 0; i < compiledColumns.Count; i++)
                {
                    ...
                    if (rowBinding != null && rowBinding.IsListElementRow && IsObjectKeyColumn(column.map) && string.IsNullOrWhiteSpace(rawValue))
                        continue;
        ...
        private void AppendFieldChangePlan(StringBuilder builder, List<string> row, List<ESTableCompiledColumn> compiledColumns, ScriptableObject owner, object rowObject, ESTableRowDirectiveInfo directive, ESTablePlanRiskSummary summary, int rowIndex, bool skipOwnerColumns = false)
        {
            ...
            for (int i = 0; i < compiledColumns.Count; i++)
            {
                ...
                {
                    AddPlanError(summary, BuildTableErrorMessage("计划预检字段", rowIndex, column.tableColumnIndex, column, owner, "原始值=" + rawValue + "；错误=" + e.Message, BuildImportCellFixSuggestion(column.valueType, column.map.valueWriteMode)));
