...
namespace ES
{
    ...
    [CreateAssetMenu(fileName = "ESSoTableDataRule", menuName = "【ES】/内容/数据表/SO 表格规则")]
    public partial class ESSoTableDataRule : ESSO
    {
        ...
        {
            var map = new Dictionary<string, ESTableColumnNameMap>(StringComparer.OrdinalIgnoreCase);
            for (int i = 0; i < enabledColumns.Count; i++)
            {
                ESTableColumnNameMap column = enabledColumns[i];
                AddColumnAlias(map, GetActiveTableColumnName(column), column);
                AddColumnAlias(map, column.columnName, column);
                AddColumnAlias(map, column.displayName, column);
                AddColumnAlias(map, column.soFieldPath, column);
            }

            return map;
        }

        private static void AddColumnAlias(Dictionary<string, ESTableColumnNameMap> map, string alias, ESTableColumnNameMap column)
        {
            if (map == null || column == null || string.IsNullOrWhiteSpace(alias))
                return;

            alias = alias.Trim();
            if (!map.ContainsKey(alias))
                map.Add(alias, column);
        }
        ...
        private static Dictionary<int, ESTableColumnNameMap> BuildTableColumnMap(List<List<string>> table, Dictionary<string, ESTableColumnNameMap> columnMap)
        {
            ...
            for (int i = 1; i < varRow.Count; i++)
            {
                ...
                if (!string.IsNullOrWhiteSpace(columnName) && columnMap.TryGetValue(columnName.Trim(), out ESTableColumnNameMap column))
                    result[i] = column;
            }
        ...
        private int FindTableColumnIndex(Dictionary<int, ESTableColumnNameMap> tableColumnMap, string columnName)
        {
            ...
            foreach (KeyValuePair<int, ESTableColumnNameMap> pair in tableColumnMap)
            {
                if (ColumnMatchesName(pair.Value, columnName))
                    return pair.Key;
            }
        ...
        private bool ColumnMatchesName(ESTableColumnNameMap column, string columnName)
        {
            if (column == null || string.IsNullOrWhiteSpace(columnName))
                return false;

            columnName = columnName.Trim();
            return string.Equals(GetActiveTableColumnName(column), columnName, StringComparison.OrdinalIgnoreCase)
                ...
                || string.Equals(column.soFieldPath, columnName, StringComparison.OrdinalIgnoreCase);
        }
