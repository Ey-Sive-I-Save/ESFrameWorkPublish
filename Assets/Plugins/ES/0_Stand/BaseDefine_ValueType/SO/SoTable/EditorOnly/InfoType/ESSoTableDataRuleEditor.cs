...
namespace ES.EditorInternal
{
    ...
    public sealed class ESSoTableDataRuleEditor : Editor
    {
            ...
            for (int i = 0; i < batches.arraySize; i++)
            {
                ...
                using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
                {
                    ...
                    if (useSuperBatch != null && useSuperBatch.boolValue)
                    {
                        ...
                        ESSoTableDataRule rule = (ESSoTableDataRule)target;
                        ESSoTableRuleUseBatch concreteBatch = rule.useBatches != null && i >= 0 && i < rule.useBatches.Count ? rule.useBatches[i] : null;
                        DrawSuperBatchQuickButtons(batch, rule, concreteBatch);
                        DrawChild(batch, "superBatchSkipInvalidRows", "跳过无效关系行");
                        DrawChild(batch, "superBatchLocateRowNumber", "定位关系表行", "输入 CSV/XLSX 里的实际行号，用于快速打开并打印这一行的解析摘要。");
                    ...
                    DrawBatchFieldFilter(batch, (ESSoTableDataRule)target);
                    ...
                    {
                        DrawBatchColumnNameSelector(batch, (ESSoTableDataRule)target, "sliceColumnName", "截取列名", "留空=使用唯一 Key 列");
                    ...
                    {
                        ESSoTableDataRule ruleForTargets = (ESSoTableDataRule)target;
                        ESSoTableRuleUseBatch concreteTargetBatch = ruleForTargets.useBatches != null && i >= 0 && i < ruleForTargets.useBatches.Count ? ruleForTargets.useBatches[i] : null;
                        DrawBatchTargetKeySelectors(batch, concreteTargetBatch);
                    }
                }
            }
        ...
        private void DrawAdvancedFields()
        {
            ...
            DrawRowBindingSelectors((ESSoTableDataRule)target);
        ...
        private void DrawSuperBatchQuickButtons(SerializedProperty batch, ESSoTableDataRule rule, ESSoTableRuleUseBatch concreteBatch)
        {
            if (batch == null || rule == null)
                return;
            ...
            {
                GUILayout.Space(EditorGUIUtility.labelWidth);
                if (GUILayout.Button("生成关系表模板", EditorStyles.miniButtonLeft, GUILayout.Width(112)))
                {
                    serializedObject.ApplyModifiedProperties();
                    ExecuteAndExit(rule, () => rule.GenerateSuperBatchTemplate(concreteBatch));
                }

                if (GUILayout.Button("打开关系表", EditorStyles.miniButtonMid, GUILayout.Width(86)))
                    OpenSuperBatchTable(batch, rule);

                if (GUILayout.Button("打开关系表文件夹", EditorStyles.miniButtonMid, GUILayout.Width(118)))
                    OpenSuperBatchTableFolder(batch, rule);

                if (GUILayout.Button("定位关系行", EditorStyles.miniButtonRight, GUILayout.Width(92)))
                    LocateSuperBatchRow(batch, rule);
            }
        }

        private void DrawBatchColumnNameSelector(SerializedProperty batch, ESSoTableDataRule rule, string childName, string label, string hint)
        {
            SerializedProperty property = batch != null ? batch.FindPropertyRelative(childName) : null;
            if (property == null)
                return;

            List<string> options = BuildColumnNameOptions(rule, true);
            DrawStringPopupProperty(property, label, options, hint);
        }

        private void DrawRowBindingSelectors(ESSoTableDataRule rule)
        {
            SerializedProperty rowBinding = serializedObject.FindProperty("rowBinding");
            if (rowBinding == null)
                return;

            SerializedProperty targetMode = rowBinding.FindPropertyRelative("targetMode");
            if (targetMode == null || targetMode.enumValueIndex != (int)ESRowTargetMode.ObjectListElement)
                return;

            DrawStringPopupProperty(rowBinding.FindPropertyRelative("rowKeyColumnName"), "行 Key 列名", BuildColumnNameOptions(rule, false), "从当前字段映射列中选择。");
            DrawStringPopupProperty(rowBinding.FindPropertyRelative("listFieldPath"), "List 字段路径", BuildListFieldPathOptions(rule), "从当前 SO/Info 类型里选择 List 或 Dictionary 字段。");
            DrawChild("rowBinding", "containerKind", "行容器类型", "Auto 会按字段类型自动识别 List/Dictionary。");
            DrawStringPopupProperty(rowBinding.FindPropertyRelative("elementKeyFieldPath"), "元素 Key 字段路径", BuildElementKeyFieldPathOptions(rule), "从 List 元素类型里选择用于匹配的 Key 字段。");
        }

        private void DrawBatchTargetKeySelectors(SerializedProperty batch, ESSoTableRuleUseBatch concreteBatch)
        {
            DrawStringPopupProperty(batch != null ? batch.FindPropertyRelative("targetGroupKey") : null, "目标 Group", BuildGroupKeyOptions(concreteBatch), "留空=不限 Group。候选来自当前批次绑定的 Group/Pack/文件夹资产。");
            DrawStringPopupProperty(batch != null ? batch.FindPropertyRelative("targetInfoKey") : null, "目标 Info", BuildInfoKeyOptions(concreteBatch), "留空=不限 Info。候选来自当前批次绑定的 Group/Pack/文件夹资产。");
        }

        private void DrawStringPopupProperty(SerializedProperty property, string label, List<string> options, string hint)
        {
            if (property == null)
                return;
            ...
            string[] display = BuildPopupDisplayOptions(options);
            using (new EditorGUILayout.HorizontalScope())
            {
                EditorGUILayout.PrefixLabel(new GUIContent(label, hint));
                int next = EditorGUILayout.Popup(selected, display);
                if (next >= 0 && next < options.Count)
                    property.stringValue = options[next];
            }
        }

        private static string[] BuildPopupDisplayOptions(List<string> options)
        {
            var display = new string[options.Count];
            for (int i = 0; i < options.Count; i++)
                display[i] = string.IsNullOrEmpty(options[i]) ? "<留空>" : options[i];
            return display;
        }

        private static List<string> BuildColumnNameOptions(ESSoTableDataRule rule, bool includeEmpty)
        {
            var result = new List<string>();
            if (includeEmpty)
                result.Add(string.Empty);
            if (rule == null || rule.columns == null)
                return result;
            ...
            {
                ESTableColumnNameMap column = rule.columns[i];
                if (column == null || !column.IsUsable)
                    continue;

                string name = string.IsNullOrWhiteSpace(column.columnName) ? column.displayName : column.columnName;
        ...
        private static List<string> BuildGroupKeyOptions(ESSoTableRuleUseBatch batch)
        {
            var result = new List<string> { string.Empty };
            var seen = new HashSet<string>(StringComparer.OrdinalIgnoreCase) { string.Empty };
            foreach (ScriptableObject asset in EnumerateBatchSourceAssets(batch))
                AddGroupKeysFromAsset(result, seen, asset);
            return result;
        }

        private static List<string> BuildInfoKeyOptions(ESSoTableRuleUseBatch batch)
        {
            var result = new List<string> { string.Empty };
            var seen = new HashSet<string>(StringComparer.OrdinalIgnoreCase) { string.Empty };
            foreach (ScriptableObject asset in EnumerateBatchSourceAssets(batch))
                AddInfoKeysFromAsset(result, seen, asset);
            return result;
        }

        private static IEnumerable<ScriptableObject> EnumerateBatchSourceAssets(ESSoTableRuleUseBatch batch)
        {
            ESSoTableRuleSourceBinding source = batch != null ? batch.sourceBinding : null;
            if (source == null)
                yield break;

            if (source.soAsset != null)
                yield return source.soAsset;
            ...
            {
                if (source.folderAssets[i] != null)
                    yield return source.folderAssets[i];
            }
        }

        private static void AddGroupKeysFromAsset(List<string> result, HashSet<string> seen, ScriptableObject asset)
        {
            if (asset is ISoDataGroup group)
            {
                AddOption(result, seen, group.FileName);
                return;
            }

            if (asset is ISoDataPack pack && pack.CachingGroups != null)
            {
                for (int i = 0; i < pack.CachingGroups.Count; i++)
                {
                    ISoDataGroup cachedGroup = pack.CachingGroups[i];
                    if (cachedGroup != null)
                        AddOption(result, seen, cachedGroup.FileName);
                }
            }
        }

        private static void AddInfoKeysFromAsset(List<string> result, HashSet<string> seen, ScriptableObject asset)
        {
            if (asset is ISoDataInfo info)
            {
                AddOption(result, seen, info.GetKey());
                return;
            }

            if (asset is ISoDataGroup group)
            {
                List<string> keys = group.AllKeys;
                if (keys == null)
                    return;

                for (int i = 0; i < keys.Count; i++)
                    AddOption(result, seen, keys[i]);
                return;
            }

            if (asset is ISoDataPack pack && pack.Keys != null)
            {
                foreach (string key in pack.Keys)
                    AddOption(result, seen, key);
            }
        }

        private static void AddOption(List<string> result, HashSet<string> seen, string value)
        {
        ...
        private static List<string> BuildListFieldPathOptions(ESSoTableDataRule rule)
        {
            var result = new List<string> { string.Empty };
            Type ownerType = rule != null && rule.typeBinding != null ? rule.typeBinding.RowOwnerType : null;
            if (ownerType == null)
                return result;

            AddMemberPathOptions(result, ownerType, string.Empty, IsRowContainerType, 2);
            return result;
        }

        private static List<string> BuildElementKeyFieldPathOptions(ESSoTableDataRule rule)
        {
            var result = new List<string> { string.Empty };
            Type ownerType = rule != null && rule.typeBinding != null ? rule.typeBinding.RowOwnerType : null;
            string listPath = rule != null && rule.rowBinding != null ? rule.rowBinding.listFieldPath : string.Empty;
            Type listType = ESRowBindingReflectionUtility.GetMemberType(ownerType, listPath);
            Type elementType = ESRowBindingReflectionUtility.GetListElementType(listType) ?? ESRowBindingReflectionUtility.GetDictionaryValueType(listType);
            if (elementType == null)
                return result;

            AddMemberPathOptions(result, elementType, string.Empty, IsKeyCandidateType, 2);
            return result;
        }

        private static void AddMemberPathOptions(List<string> result, Type type, string prefix, Func<Type, bool> predicate, int depth)
        {
            if (result == null || type == null || depth < 0)
                return;
            ...
            for (int i = 0; i < members.Length; i++)
            {
                ...
                Type valueType = GetEditorMemberType(member);
                if (valueType == null || IsIgnoredEditorMember(member))
                    continue;

                string path = string.IsNullOrEmpty(prefix) ? member.Name : prefix + "." + member.Name;
                if (predicate(valueType) && !result.Contains(path))
                    result.Add(path);

                if (depth > 0 && ShouldExpandEditorMember(valueType))
                    AddMemberPathOptions(result, valueType, path, predicate, depth - 1);
            }
        }

        private static Type GetEditorMemberType(MemberInfo member)
        {
        ...
        private static bool IsIgnoredEditorMember(MemberInfo member)
        {
            if (member == null || member.Name.StartsWith("<", StringComparison.Ordinal))
                return true;
        ...
        private static bool IsRowContainerType(Type type)
        {
            return type != null && (typeof(IList).IsAssignableFrom(type) || typeof(IDictionary).IsAssignableFrom(type));
        }

        private static bool IsKeyCandidateType(Type type)
        {
            if (type == null)
                return false;
        ...
        private static bool ShouldExpandEditorMember(Type type)
        {
            if (type == null || type == typeof(string) || type.IsPrimitive || type.IsEnum)
                return false;
            if (typeof(UnityEngine.Object).IsAssignableFrom(type) || IsRowContainerType(type))
                return false;
            return type.IsClass || (type.IsValueType && !type.IsPrimitive);
        }

        private void DrawBatchFieldFilter(SerializedProperty batch, ESSoTableDataRule rule)
        {
            SerializedProperty activeFields = batch != null ? batch.FindPropertyRelative("activeFields") : null;
            SerializedProperty excludedFields = batch != null ? batch.FindPropertyRelative("excludedFields") : null;
            if (activeFields == null || excludedFields == null || rule == null)
                return;

            EditorGUILayout.LabelField("字段过滤", _sectionTitleStyle);
            EditorGUILayout.LabelField("留空表示全部字段生效。日常使用直接勾选，不需要手填字段名；超级批关系表仍可用 activeFields/excludedFields 覆盖。", _mutedStyle);

            List<ESTableColumnNameMap> usableColumns = GetUsableFilterColumns(rule);
            if (usableColumns.Count == 0)
            {
                EditorGUILayout.HelpBox("当前没有可用字段映射，先在字段映射区生成或配置列。", MessageType.Info);
                return;
            }

            HashSet<string> activeSet = ParseFieldFilterTokens(activeFields.stringValue);
            HashSet<string> excludedSet = ParseFieldFilterTokens(excludedFields.stringValue);
            bool hasActiveLimit = activeSet.Count > 0;

            using (new EditorGUILayout.HorizontalScope())
            {
                GUILayout.Space(EditorGUIUtility.labelWidth);
                if (GUILayout.Button("全部参与", EditorStyles.miniButtonLeft, GUILayout.Width(80)))
                {
                    activeFields.stringValue = string.Empty;
                    excludedFields.stringValue = string.Empty;
                    activeSet.Clear();
                    excludedSet.Clear();
                    hasActiveLimit = false;
                }
                if (GUILayout.Button("清空参与", EditorStyles.miniButtonMid, GUILayout.Width(80)))
                {
                    activeFields.stringValue = "__none__";
                    activeSet = ParseFieldFilterTokens(activeFields.stringValue);
                    hasActiveLimit = true;
                }
                if (GUILayout.Button("清空排除", EditorStyles.miniButtonRight, GUILayout.Width(80)))
                {
                    excludedFields.stringValue = string.Empty;
                    excludedSet.Clear();
                }
            ...
            {
                ESTableColumnNameMap column = usableColumns[i];
                string token = GetFieldFilterToken(column);
                if (string.IsNullOrWhiteSpace(token))
                    continue;

                bool included = !hasActiveLimit || FieldFilterMatches(column, activeSet);
                bool excluded = FieldFilterMatches(column, excludedSet);
                using (new EditorGUILayout.HorizontalScope())
                {
                    bool newIncluded = EditorGUILayout.ToggleLeft(BuildFieldFilterLabel(column), included, GUILayout.MinWidth(260));
                    bool newExcluded = GUILayout.Toggle(excluded, "排除", EditorStyles.miniButton, GUILayout.Width(52));

                    if (newIncluded != included)
                    {
                        if (!hasActiveLimit)
                        {
                            activeSet = BuildAllFieldTokenSet(usableColumns);
                            hasActiveLimit = true;
                        }
                        ...
                        if (newIncluded)
                            ...
                            RemoveFieldFilterAliases(activeSet, column);
                    }
                    ...
                    if (newExcluded != excluded)
                    {
                        if (newExcluded)
                            ...
                            RemoveFieldFilterAliases(excludedSet, column);
                    }
                }
            }
            EditorGUI.indentLevel--;

            activeFields.stringValue = hasActiveLimit ? JoinFieldFilterTokens(activeSet) : string.Empty;
            excludedFields.stringValue = JoinFieldFilterTokens(excludedSet);

            using (new EditorGUILayout.HorizontalScope())
            {
                EditorGUILayout.PrefixLabel("当前过滤");
                EditorGUILayout.SelectableLabel(
                    "参与=" + FormatFieldFilterSummary(activeFields.stringValue)
                    + " | 排除=" + (string.IsNullOrWhiteSpace(excludedFields.stringValue) ? "无" : excludedFields.stringValue),
                    _mutedStyle,
                    GUILayout.Height(EditorGUIUtility.singleLineHeight));
            }
        }

        private static List<ESTableColumnNameMap> GetUsableFilterColumns(ESSoTableDataRule rule)
        {
            ...
            {
                ESTableColumnNameMap column = rule.columns[i];
                if (column != null && column.IsUsable && !string.IsNullOrWhiteSpace(GetFieldFilterToken(column)))
                    result.Add(column);
            }
        ...
        private static HashSet<string> BuildAllFieldTokenSet(List<ESTableColumnNameMap> columns)
        {
            ...
            {
                string token = GetFieldFilterToken(columns[i]);
                if (!string.IsNullOrWhiteSpace(token))
                    result.Add(token);
            }
        ...
        private static HashSet<string> ParseFieldFilterTokens(string text)
        {
            var result = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        ...
        private static string JoinFieldFilterTokens(HashSet<string> tokens)
        {
        ...
        private static string FormatFieldFilterSummary(string value)
        {
            if (string.IsNullOrWhiteSpace(value))
                return "全部";
            return string.Equals(value.Trim(), "__none__", StringComparison.OrdinalIgnoreCase) ? "无字段" : value;
        }

        private static string GetFieldFilterToken(ESTableColumnNameMap column)
        {
            if (column == null)
                return string.Empty;
            if (!string.IsNullOrWhiteSpace(column.columnName))
                return column.columnName.Trim();
            if (!string.IsNullOrWhiteSpace(column.soFieldPath))
                return column.soFieldPath.Trim();
            return !string.IsNullOrWhiteSpace(column.displayName) ? column.displayName.Trim() : string.Empty;
        }

        private static bool FieldFilterMatches(ESTableColumnNameMap column, HashSet<string> tokens)
        {
            if (column == null || tokens == null || tokens.Count == 0)
                return false;

            return ContainsFieldFilterToken(tokens, column.columnName)
                || ContainsFieldFilterToken(tokens, column.soFieldPath)
                || ContainsFieldFilterToken(tokens, column.displayName);
        }

        private static bool ContainsFieldFilterToken(HashSet<string> tokens, string value)
        {
            return !string.IsNullOrWhiteSpace(value) && tokens.Contains(value.Trim());
        }

        private static void RemoveFieldFilterAliases(HashSet<string> tokens, ESTableColumnNameMap column)
        {
            if (tokens == null || column == null)
                return;

            RemoveFieldFilterToken(tokens, column.columnName);
            RemoveFieldFilterToken(tokens, column.soFieldPath);
            RemoveFieldFilterToken(tokens, column.displayName);
        }

        private static void RemoveFieldFilterToken(HashSet<string> tokens, string value)
        {
            if (!string.IsNullOrWhiteSpace(value))
                tokens.Remove(value.Trim());
        }

        private static string BuildFieldFilterLabel(ESTableColumnNameMap column)
        {
            string token = GetFieldFilterToken(column);
            string display = string.IsNullOrWhiteSpace(column.displayName) ? string.Empty : column.displayName.Trim();
            string path = string.IsNullOrWhiteSpace(column.soFieldPath) ? string.Empty : column.soFieldPath.Trim();
            if (!string.IsNullOrEmpty(display) && !string.Equals(display, token, StringComparison.OrdinalIgnoreCase))
                token += "  (" + display + ")";
            if (!string.IsNullOrEmpty(path) && !string.Equals(path, token, StringComparison.OrdinalIgnoreCase))
                token += "  -> " + path;
            return token;
        }
        ...
        private static void OpenSuperBatchTable(SerializedProperty batch, ESSoTableDataRule rule)
        {
            string path = BuildSuperBatchTableFullPath(batch);
            if (File.Exists(path))
            {
                EditorUtility.OpenWithDefaultApp(path);
                return;
            ...
            OpenSuperBatchTableFolder(batch, rule);
            Debug.LogWarning("超级批关系表不存在，已打开关系表文件夹：" + path, rule);
        }

        private static void OpenSuperBatchTableFolder(SerializedProperty batch, ESSoTableDataRule rule)
        {
            string path = BuildSuperBatchTableFullPath(batch);
        ...
        private static void LocateSuperBatchRow(SerializedProperty batch, ESSoTableDataRule rule)
        {
            if (rule == null)
                return;
            ...
            int rowNumber = 1;
            SerializedProperty rowProperty = batch != null ? batch.FindPropertyRelative("superBatchLocateRowNumber") : null;
            if (rowProperty != null)
                rowNumber = Math.Max(1, rowProperty.intValue);

            string tablePath = GetStringChild(batch, "superBatchTablePath");
            string report = rule.BuildSuperBatchRelationRowReport(tablePath, rowNumber);
            EditorGUIUtility.systemCopyBuffer = "关系表：" + path + Environment.NewLine + "行号：" + rowNumber;
            Debug.Log(report + Environment.NewLine + "已复制关系表路径和行号到剪贴板。", rule);
        }
        ...
        private static void OpenBatchOutputFolder(SerializedProperty batch)
        {
            ...
            if (fileKind != null && fileKind.enumValueIndex == (int)ESTableFileKind.Csv)
                relativeFolder = FirstNotEmpty(GetStringChild(batch, "csvRelativePath"), "csv");
            else if (fileKind != null && fileKind.enumValueIndex == (int)ESTableFileKind.Xlsx)
                relativeFolder = FirstNotEmpty(GetStringChild(batch, "xlsxRelativePath"), "xlsx");

            string folder = string.IsNullOrEmpty(relativeFolder)
                ? Path.GetFullPath(Path.Combine(Application.dataPath, "..", root))
                : Path.GetFullPath(Path.Combine(Application.dataPath, "..", root, relativeFolder));
            Directory.CreateDirectory(folder);
            EditorUtility.OpenWithDefaultApp(folder);
        }
        ...
        private static string BuildSuperBatchTableFullPath(SerializedProperty batch)
        {
