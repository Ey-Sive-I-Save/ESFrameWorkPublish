...
Buff 配置 / 代码创建
        ...
        -> 当前实体实现由 Entity 创建 EffectLease 并取得 OwnerId
        -> 每个运行期实例持有自己的 Token
...
```csharp
ESFloatValueChangeSet moveSpeed = entity.GetFloatStat("MoveSpeed", 5f);
...
float finalSpeed = entity.GetFloatStatValue("MoveSpeed", 5f);
...
```csharp
bool mayJump = entity.GetPermitValue("MayJump", true);
ESPermitLawResult reason = entity.GetPermitResult("MayJump", true);
```
...
`Entity.ClearValueChanges()` 会清空实体内所有 Set 并使已有 Token 失效，适用于显式重置。有活动 `EffectLease` 时它会拒绝执行；必须先释放 Lease，避免活跃 Buff 持有失效 Token。清理/重绑定期间也禁止创建新 Lease，防止 `Changed` 回调在清理枚举中插入新的运行期所有权。实体销毁是唯一允许强制失效活跃 Lease 的路径：它先让全部 Lease 失效，再逐个清理 Set；单个 `Changed` 监听器的异常会被记录且不会阻断剩余清理。

`ownerId` / `sourceId` 的 `0` 表示未归属，不会建立批量回收索引；`ReleaseAllByOwner(0)` 与 `ReleaseAllBySource(0)` 都不会释放任何修正器。需要批量清理时必须传入非零的领域内拥有者/来源 ID。

## 底层边界

ValueChange 是 ES 的可撤销修正器内核，不是 Buff 专属系统，也不持有任何角色、资源或配置状态。角色的物理所有者是 `Entity`，不引入额外的 AttributeDomain：

```text
Entity: 属性 ID 路由、基础值缓存、固定/稀疏槽位、EffectLease 所有权
    -> ValueChange: Token、Float/Permit 裁决、缓存、批处理
        -> Buff: 创建、更新、释放自己的效果
```

`ESEffectLease` 是宿主级 API 的零分配生命周期契约。`Entity` 持有实际 Set 与 Lease Slot；Buff 实例只保存自己的 Lease 和 Token。Lease 只保存 Entity Slot 与 Generation，重复或过期释放由 Entity 拒绝。释放期间槽位不会复用，防止通知回调内的新效果与旧 OwnerId 混淆。
...
表也支持任意自定义 Key，供生命、攻击、防御等后续业务模块注册并通过 `Entity` 读取；Buff 不持有或篡改这些基础值。
