Shader "ES/UI/Composite URP"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 _MainTex_ST; float4 _MainTex_TexelSize; half4 _Color; float4 _MainTexScaleOffset; float _TimeMode; float _CustomTime; float _TimeScale; float _VertexColorStrength; float _EnableFlow; float4 _FlowSpeed; float _FlowStrength; float _EnableShine; half4 _ShineColor; float _ShineSpeed; float _ShineWidth; float _ShineAngle; float _ShineIntensity; float _EnableSparkle; half4 _SparkleColor; float _SparkleScale; float _SparkleSpeed; float _SparkleDensity; float _SparkleSharpness; float _SparkleIntensity; float _EnableChromatic; float _ChromaticOffset; float _ChromaticIntensity; float _ChromaticEdgeOnly; float _ChromaticAngle; float _EnableBlur; float _BlurRadius; float _BlurIntensity; float _EnableHologram; half4 _HologramColor; float _HologramFrequency; float _HologramSpeed; float _EnableGlitch; float _GlitchAmount; float _GlitchSpeed; float _AlphaClip; float _Cutoff; half4 _TextureSampleAdd; float4 _ClipRect;
            CBUFFER_END
            ...
            float ESGet2DClipping(float2 position, float4 clipRect) { float2 inside=step(clipRect.xy,position)*step(position,clipRect.zw); return inside.x*inside.y; }
