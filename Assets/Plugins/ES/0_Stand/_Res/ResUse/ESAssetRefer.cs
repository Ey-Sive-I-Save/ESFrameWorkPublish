...
namespace ES
{
    ...
    /// </summary>
    [Serializable]
    public class ESAssetRefer<T> : ESAssetReferBase where T : UnityEngine.Object
    {
        #region 编辑器支持
        ...
        public void LoadAsync(ESResLoader loader, Action<bool, T> onComplete, bool autoStartLoading = true)
        {
            if (!IsValid)
            {
                UnityEngine.Debug.LogError("[ESAssetRefer] 无效的资源引用，GUID为空");
            ...
            targetLoader.AddAsset2LoadByGUIDSourcer(_guid, (success, source) =>
            {
                ...
                {
                    UnityEngine.Debug.LogError($"[ESAssetRefer] 加载失败: GUID={_guid}");
                    onComplete?.Invoke(false, default(T));
                }
            });
        ...
        public bool LoadSync(ESResLoader loader, out T asset)
        {
            ...
            if (!success)
            {
                UnityEngine.Debug.LogError($"[ESAssetRefer] 同步加载失败: GUID={_guid}");
            }
        ...
        public void InstantiateAsync(Action<GameObject> onComplete, Transform parent = null, ESResLoader loader = null)
        {
            ...
            {
                UnityEngine.Debug.LogError("[ESAssetRefer] InstantiateAsync只能用于GameObject类型");
    ...
    public class ESAssetReferScene : ESAssetReferBase
    {
        ...
        public override void Draw()
        {
            ...
            if (!string.IsNullOrEmpty(_guid))
            {
                current = ESStandUtility.SafeEditor.LoadAssetByGUIDString(_guid);
            }

            var newScene = EditorGUILayout.ObjectField(current, typeof(SceneAsset), false);
            if (newScene == current)
            {
                return;
            }
            ...
            _guid = ESStandUtility.SafeEditor.GetAssetGUID(newScene);
        ...
        public void LoadAsync(ESResLoader loader, Action<bool> onComplete, bool autoStartLoading = true)
        {
            if (!IsValid)
            {
                UnityEngine.Debug.LogError("[ESAssetReferScene] 无效的场景引用，GUID为空");
                onComplete?.Invoke(false);
                return;
            }

            var targetLoader = loader ?? ESResMaster.GlobalResLoader;
            if (ESResMaster.GlobalAssetKeys.TryGetESResKeyByGUID(_guid, out var sceneKey))
            {
                targetLoader.Add2LoadByKey(sceneKey, ESResSourceLoadType.ABScene, (success, _) => onComplete?.Invoke(success));
                if (autoStartLoading)
                {
                    targetLoader.LoadAllAsync();
                }
            ...
            {
                UnityEngine.Debug.LogError($"[ESAssetReferScene] 未找到场景资源键: GUID={_guid}");
                onComplete?.Invoke(false);
            }
        }
