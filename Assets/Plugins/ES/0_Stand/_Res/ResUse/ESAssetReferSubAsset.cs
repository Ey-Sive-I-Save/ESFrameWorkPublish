...
namespace ES
{
    ...
    /// </summary>
    [Serializable]
    public class ESAssetReferSubAsset<TMain, TSub> : ESAssetReferBase 
        ...
        where TSub : UnityEngine.Object
    {
        ...
        public override void Draw()
        {
            ...
            if (newMainAsset != _editorMainAsset)
            {
                ...
                {
                    UnityEngine.Debug.LogWarning($"[ESAssetReferSubAsset] 主资产类型不匹配：需要 {typeof(TMain).Name}，但拖入的是 {newMainAsset.GetType().Name}");
        ...
        private void RefreshSubAssets()
        {
            ...
            {
                UnityEngine.Debug.LogWarning($"[ESAssetReferSubAsset] 主资产 '{_editorMainAsset.name}' 中没有找到类型为 {typeof(TSub).Name} 的子资产");
            }
        }
        
        /// <summary>
        /// 检测资产收集状态（与 ESAssetRefer 保持一致）
        /// </summary>
        private void TryAutoCollectAsset(UnityEngine.Object asset)
        {
            ...
            try
            {
                ...
                {
                    UnityEngine.Debug.LogWarning($"[ESAssetReferSubAsset] 无法获取资产路径: {asset.name}");
                    return;
                }
                ...
                {
                    UnityEngine.Debug.Log($"[ESAssetReferSubAsset] 资产已收集: {asset.name} -> AB: {importer.assetBundleName}");
                    return;
                }
                
                UnityEngine.Debug.LogWarning(
                    $"[ESAssetReferSubAsset] 资产尚未收集到ES系统\n" +
                    $"资产名: {asset.name}\n" +
                    $"路径: {assetPath}\n" +
                ...
                if (openEditor)
                {
                    ...
                    {
                        UnityEngine.Debug.LogWarning("[ESAssetReferSubAsset] 无法找到 ES 资源管理窗口菜单项，请手动打开【ES】/框架/资源管线/资源管理窗口。");
                    }
            ...
            {
                UnityEngine.Debug.LogError($"[ESAssetReferSubAsset] 资产收集状态检测失败: {ex.Message}\n{ex.StackTrace}");
            }
        }
#endif
        ...
        public void LoadAsync(ESResLoader loader, Action<bool, TSub> onComplete, bool autoStartLoading = true)
        {
            if (!IsValid)
            {
                UnityEngine.Debug.LogError("[ESAssetReferSubAsset] 无效的资源引用，GUID或子资产名称为空");
            ...
            targetLoader.AddAsset2LoadByGUIDSourcer(_guid, (success, source) =>
            {
                ...
                {
                    UnityEngine.Debug.LogError($"[ESAssetReferSubAsset] 主资产加载失败: GUID={_guid}");
                    onComplete?.Invoke(false, default(TSub));
                    return;
                }
                ...
                {
                    UnityEngine.Debug.LogError($"[ESAssetReferSubAsset] 子资产未找到: {_subAssetName} in {source.Asset.name}");
                    onComplete?.Invoke(false, default(TSub));
                }
            });
        ...
        public bool LoadSync(ESResLoader loader, out TSub asset)
        {
            ...
            if (!success)
            {
                UnityEngine.Debug.LogError($"[ESAssetReferSubAsset] 同步加载失败: GUID={_guid}, SubAsset={_subAssetName}");
            }
        ...
        private TSub FindSubAsset(UnityEngine.Object mainAsset)
        {
            ...
            UnityEngine.Debug.LogWarning($"[ESAssetReferSubAsset] 运行时子资产加载尚未完全实现: {_subAssetName}");
#endif
