...
4. 实网写入必须重新 planTask，并绑定用户当前批准、Actor、InvocationId、租户、应用身份 Hash、目标清单/任务、输入 Hash、AICommand/TaskContract/Worker/Schema Hash、60 秒超时和停止条件。
5. 创建使用官方 client_token 或带 InvocationId 后缀的可恢复清单名；更新必须携带 fresh expectedUpdatedAt。
