# OpTargetPack 与 Item 运动检查 AI 命令
...
```text
Assets/Plugins/ES/AIWarnings/30_运行时专项（RuntimeOperations）/Item与Shot物理（ItemShotPhysics）/OpTargetPack_Item整合与池化重设_AI协作警告.md
Assets/Plugins/ES/AIWarnings/30_运行时专项（RuntimeOperations）/Item与Shot物理（ItemShotPhysics）/运动职责_Shot必中与Item运动_AI协作警告.md
```

## 执行要求

```text
检查 Shot 必中、Item 运动、OpTargetPack、对象池、命中、Tick 的职责边界。
```

## ContractCompleteness

```yaml
commandId: item-motion.review
writeMode: read-only
