...
命令类型：只读体检。
默认改文件：否，修复绑定/过滤需用户确认。
风险等级：L1/L2。

## 必须先读

```text
Assets/Plugins/ES/AIWarnings/InputRuntime/输入与交互入口_AI协作警告.md
...
```text
检查输入动作是否在正确 RuntimeMode 下响应，禁止业务脚本硬编码按键绕过 ESInput。
```
