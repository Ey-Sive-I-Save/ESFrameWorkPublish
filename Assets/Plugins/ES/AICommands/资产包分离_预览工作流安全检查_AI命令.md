...
```text
检查 ESAssetPackagePreviewWorkflow、ReloadDomain 清理、小格子帧缓存、大预览实时渲染、缓存不进 Assets、禁止旧方案。
```

## ContractCompleteness

```yaml
commandId: asset-package.preview.review
writeMode: read-only
...
3. 改动文件：没有改文件就写“无”。
4. 验证结果：修复时编译 Assembly-CSharp-Editor.csproj
5. 剩余风险：列出仍需人工确认的点。
```
