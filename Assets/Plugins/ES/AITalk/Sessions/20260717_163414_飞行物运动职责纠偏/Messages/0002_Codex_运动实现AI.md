...
命中检测：
  可由 ItemShotModule 调用物理查询得到 HitCandidate，但只报告候选，不直接结算伤害。

命中效果：
  由战斗/技能/Operation/Buff 系统处理，消费 HitCandidate 或 ShotHitEvent。
...
2. 把 ESShotMotion 从独立 MonoBehaviour 行为逐步收敛为 ItemShotModule；MonoBehaviour 可以只保留为临时桥接或快速测试入口。
3. 定义 ShotMotionResult / HitCandidate / ExpireReason 这类纯结果结构，让上层系统消费，而不是运动模块直接做伤害/VFX/池回收。
```
...
4. 不要让 ItemShotModule 直接依赖玩家输入、技能释放条件或伤害公式。
5. 未经用户授权，不改代码。
```
