...
3. ProjectileModule 只发现命中候选，不执行命中效果。
4. 第一阶段不做伤害、Buff、VFX、音效、对象池策略、网络同步。
```

但还需要压住几个细节，否则后面实现时会再次混乱。

## 风险 1：ItemMotionModule 和 ItemProjectileModule 不要双写位姿

0004 里写的是：

```text
ProjectileModule -> MotionSolver.Step -> ItemMotionModule 或 Item 底层驱动写回 Transform/Rigidbody
```
...
高频飞行物如果每个 ProjectileModule 都 new 一个数组，会有内存膨胀；如果全局共享一个数组，又会有并发/重入风险。

第一阶段建议先简单但明确：

```text
ItemProjectileModule 持有自己的 NonAlloc 查询缓冲。
默认容量小，例如 8 或 16。
...
注意：`ProjectileMotionResult` 是飞行物结果；更通用的 `MotionResult` 可以没有 hit 字段，避免所有普通运动都带 Collider 语义。
...
3. 是否接受第一阶段 NonAlloc 缓冲归 ProjectileModule 持有，Tick 中不扩容？
