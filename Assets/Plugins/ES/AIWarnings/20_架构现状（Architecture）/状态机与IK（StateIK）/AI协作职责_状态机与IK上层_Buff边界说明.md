...
`StateFinalIKDriver` 可以作为状态表现桥留在基础模板中，但它不是 Solver 本身：

- 基础模板与通用池模板没有对应 Solver 时，必须显式关闭相关 `enable*IK` / `enableHitReaction` / `enableRecoil` 开关。
- 正式角色 Variant 只要开启一项能力，必须同时挂齐对应 FinalIK Solver 并通过模板验证；Driver 在 Bind 时发现缺失必须报错，不能静默 no-op。
- 不能用 `autoAdd` 掩盖生产装配错误；Solver、骨骼和性能预算应在 Variant 制作阶段明确。
...
`EntityBuffDomain` 已不是空域：当前已有运行时 Buff 实例、叠层、来源/持续时间、ValueChange / Permit 与 OpSupport 关联入口。禁止继续传播“空域”结论，也不要把这句话扩大为“完整商业 Buff 系统已经完成”；事件覆盖、完整驱散策略、诊断和玩法验收仍需按当前源码确认。

当前职责链已经是：

```text
EntityBuffDomain
  → Add / Merge / Refresh / Tick / Remove / Query
  → ESActiveBuffRuntime
      → ValueChange EffectLease
      → Tag LeaseSet
      → Context 依赖
      → Start / Tick / Remove Op
      → Deactivate 后归还自身全部持有
```

边界必须同时成立：

- BuffDomain 只拥有 Buff 的时长、层数、冲突/合并、实例、触发 Op 和回收流程。
- 属性表、固定/稀疏 ValueChange Set、Tag 聚合容器和 EffectLease Slot 的权威宿主是 `Entity`，不是 BuffDomain。
- `_EntityBuffDomain.cs` 中现存 ValueChange 转发属于迁移兼容面，不允许据此恢复 BuffDomain 内第二套属性运行时。
- 每个 `ESActiveBuffRuntime` 只释放自己的 EffectLease、Tag Lease 和 Context 订阅；禁止按 Tag 或属性键强制清除其他来源。
- Entity 对象池复用、固有 Tag、装备/区域 Lease、存档/网络恢复与 PlayMode 压测仍需独立验收。
