...
> 状态：现行约束（含 `es.scene.scan@1` 分阶段原型源码）。
> 最后核对：2026-08-08。
...
5. 运行记录必须包含操作者、RunId、TaskId/版本、Git commit、Unity/Worker 版本、输入 Manifest Hash、输出 Hash 和错误；当前 RunRecord 仅是数据模型，尚未持久化。
...
- C# Editor 管理骨架：已建立；TaskContract 的共享字段已按 JSON Schema 对齐，未注册 Worker 时 ProcessRunner 拒绝构造可执行命令。受管进程已补任务级超时上限、Job Object/进程树回退、stdout/stderr 有界异步排空、ReloadDomain 全局清理和同 RunId 并发拒绝；自动往返与故障注入测试仍待实现。
