...
## P0：ESGenericLife 与 Pool 生命周期分部

`ESGenericLife` 是根 GameObject 的通用生命周期组织器；对象池只是它当前已落地的一个功能分部。禁止把它降级为 Entity 专属、Item 专属或单纯 Pool 包装，也禁止凭空创建没有实际调用者的泛化生命周期大接口。
...
- 一个根节点只有一个主 Pool 生命周期接收者，且必须与 `ESGenericLife` 同根。
- 允许 `RegisterPoolExtension(...)` 注入多个独立扩展；每个 Extension 具体类型只能有一个。扩展不要求 Entity 直接引用，也不通过扫描子节点发现。
- Spawn 顺序为 Root → Extensions；Despawn 为 Extensions 逆序 → Root。
- 注册/注销只允许在 inactive 状态；不得在生命周期回调内改变接收者列表。
- `GetComponentsInChildren`、每次借还扫描层级、缓存整棵树接收者列表均为 P0 禁止。
- 新建和预热实例必须先进入 inactive 并执行一次 Despawn 基线，再进入 inactive 队列。`OnPoolSpawned` 必须在 `SetActive(true)` 前调用。
- Unity 的 `Awake` 无法由 Pool 延后；Awake 只能做本地准备，不能假定已被借出或已完成 Pool/Tag/定义绑定。
- 接收者异常不能让实例丢失。Pool 账本必须用 `try/finally` 收口到 inactive 或明确销毁；Spawn 内归还请求必须延迟到 Spawn 派发结束后。
- 多 Root 接收者、跨根序列化 Root、重复 Extension 均是配置错误：开发/构建门禁必须拒绝，正式运行时不得静默任选一个。

详细契约见 `Documentation/ES_GENERIC_LIFE.md`。
