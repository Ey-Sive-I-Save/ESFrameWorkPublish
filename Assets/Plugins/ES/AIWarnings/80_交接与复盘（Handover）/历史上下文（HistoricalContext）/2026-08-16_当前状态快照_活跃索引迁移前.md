# 2026-08-16 AIWarnings 当前状态快照（活跃索引迁移前）

> 状态：历史快照。
> 迁移说明：此文件保留迁移时的原始工作上下文，不再是首次必读入口，也不授予当前实现、验证或发布权限。新的活跃入口为 `00_开始阅读（Start）/当前状态（CurrentStatus）.md`。
...
- 当前验证证据：Unity 2022.3.45f1 完成脚本导入与 Domain Reload，Console 无编译错误；实例表、Item、Attachment、Equipment 四组定向 EditMode Job `f9f317312e3e49d882662a4c1f11e95b` 为 30/30，通过跨泛型表 Token 隔离、实例 Handle、swap-remove、持久 ID 跳过、Item 转移、基础/专项投影、失败回滚、装备/卸下、普通 Item 拒绝进入 Weapon Slot、活动 Attachment 过渡期间关系变更门禁、动画绑定等待、挂点代际和 Entity 回池清理；统一内容注册 Job `addbfbcb03ba4bfdab49c12df253341a` 为 19/19。Entity 直接销毁路径已有源码前置清理，但尚无 PlayMode 销毁证据；也尚无 Profiler、Player 或发布证据。
