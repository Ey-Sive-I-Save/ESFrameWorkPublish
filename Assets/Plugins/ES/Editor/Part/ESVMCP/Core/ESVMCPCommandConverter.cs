...
namespace ES.VMCP
{
    ...
    {
        private static Dictionary<string, Type> _commandTypeMap=new Dictionary<string, Type>();
        ...
        /// </summary>
        /// 
        public class EditorRegister_SubClass : EditorRegister_FOR_AsSubclass<ESVMCPCommandBase>
        {
            public override int Order => 5;

            public override void Handle(Type subClassType)
            {
                if(_commandTypeMap==null)
                    _commandTypeMap=new Dictionary<string, Type>();

                    
                var attribute = subClassType.GetCustomAttribute<ESVMCPCommandAttribute>();
                if (attribute != null && !string.IsNullOrEmpty(attribute.CommandType))
                {
                    if (!_commandTypeMap.ContainsKey(attribute.CommandType))
                    {
                        _commandTypeMap[attribute.CommandType] = subClassType;
                      //  Debug.Log($"[ESVMCP] 注册命令类型: {attribute.CommandType} -> {subClassType.Name}");
                    } 
                    else
                    {
                      //  Debug.LogWarning($"[ESVMCP] 命令类型 {attribute.CommandType} 已经注册，跳过重复注册 {subClassType.Name}");
                    }
                }
            }
        }
