...
namespace ES
{
    ...
    /// 支持操作的动态加载、执行和取消，为复杂的业务逻辑提供灵活的组合能�?
    /// 
    /// 【架构演进历程�?
    /// �?旧标�?OBSOLUTE): 采用针化操作模式 On/From/With 三参数链式调�?
    ///   - 优势: 链式语法直观，参数职责明�?
    ///   - 缺陷: 类型参数固定，扩展性受�?
    /// 
    /// �?新标�?Current): 采用实时目标+依赖未定上下文模�?
    ///   - 优势: 目标类型灵活，上下文可动态注�?
    ///   - 特点: 不依赖确定的类型参数，支持运行时类型决策
    /// 
    /// 【设计原则�?
    /// �?单一职责: 每个Operation只负责一种特定的输出逻辑
    /// �?可组合�? 支持多个Operation的组合和链式执行
    /// �?可取消�? 提供操作取消机制，支持事务性回�?
    /// �?类型安全: 通过泛型约束保证编译时类型检�?
    /// </summary>
    /// <summary>
    #region  旧标准输出操作接口说�?
    /// 旧标准输出操作接�?(OBSOLETE - 已废�?
    /// 
    /// 【设计理念�?
    /// 基于针化操作模式的三参数设计:
    /// �?On: 操作目标对象，明确指定操作的作用目标
    /// �?From: 数据来源对象，提供操作所需的输入数�?
    ...
    /// �?缺乏对动态类型和运行时上下文的支�?
    /// </summary>
    /// <typeparam name="On">操作目标类型 - 接受操作影响的对�?/typeparam>
    /// <typeparam name="From">数据来源类型 - 提供操作数据的对�?/typeparam>
    /// <typeparam name="With">操作参数类型 - 携带操作执行参数的对�?/typeparam>
    [System.Obsolete("此接口已废弃，请使用IOutputOperation<Target,Logic>新标准接�?)]
    ...
    #endregion
    /// <summary>
    /// 新标准输出操作接�?(Current Standard)
    /// 
    /// 【设计理念�?
    /// 基于实时目标+逻辑上下文的双参数设�?
    /// �?Target: 操作的实时目标对象，支持运行时类型决�?
    /// �?Logic: 逻辑上下文对象，提供操作执行所需的依赖和环境
    /// 
    /// 【核心优势�?
    /// �?灵活的目标类�? Target可以是任意类型，支持多态和动态绑�?
    /// �?丰富的逻辑上下�? Logic承载复杂的执行环境和依赖关系
    /// �?简化的参数模型: 相比三参数模式，减少了参数传递的复杂�?
    /// �?更好的扩展�? 易于添加新的目标类型和逻辑上下�?
    /// 
    /// 【典型使用场景�?
    /// <code>
    /// // 示例: 属性修改操�?
    ...
    ///     target: affectedUnits,            // 目标: 受影响单位列�?
    ///     logic: skillExecutionContext     // 逻辑: 技能执行上下文
    /// );
    /// </code>
    /// 
    /// 【与旧标准对比�?
    /// | 方面 | 旧标�?On/From/With) | 新标�?Target/Logic) |
    /// |------|---------------------|---------------------|
    /// | 参数数量 | 3个固定参�?| 2个灵活参�?|
    /// | 类型灵活�?| 编译时确�?| 运行时决�?|
    /// | 扩展�?| 受限 | 优秀 |
    ...
    /// <typeparam name="Logic">逻辑上下文类�?- 提供操作执行环境和依赖的对象类型</typeparam>
    public interface IOutputOperation<Target, Logic> : IOperation<Logic, Target>
    {
        ...
        /// �?触发取消操作的相关事�?
        /// 
        /// 注意: 并非所有操作都支持取消，具体实现需要根据业务需求决�?
        /// </summary>
        /// <param name="target">需要撤销操作影响的目标对�?/param>
    ...
    /// �?默认取消行为: 提供DefaultAction作为取消操作的默认实�?
    /// 
    /// 【使用场景示例�?
    /// <code>
    /// // 委托操作示例
    ...
    public abstract class OutputOperation_MustCancel<Target, Logic> : IOutputOperation<Target, Logic>
    {
        ...
        /// 建议在实现中记录必要的状态信息，以便TryCancel时进行准确回�?
        /// </summary>
        /// <param name="target">操作目标对象</param>
        /// <param name="logic">逻辑上下文对�?/param>
        ...
        /// 实现时应�?
        /// �?撤销TryOperation中执行的所有操�?
        /// �?恢复目标对象的原始状�?
        /// �?调用OnCancel委托通知外部代码
        /// �?释放占用的资�?
        /// </summary>
        /// <param name="target">需要撤销操作的目标对�?/param>
    ...
    �?                                                                              �?
    �?【设计模式�?                                                                  �?
    �?�?策略模式: 不同的Operation实现不同的操作策�?                                 �?
    �?�?命令模式: Operation封装了操作请求，支持撤销和重�?                           �?
    �?�?模板方法: 抽象基类定义操作流程，子类实现具体细�?                           �?
    �?                                                                              �?
    �?【类型安全�?                                                                  �?
    �?�?泛型约束确保编译时类型检�?                                                  �?
    ...
    �?2. 继承OutputOperation_MustCancel实现可取消操�?                             �?
    �?3. 根据业务需求定义Target和Logic的具体类�?                                   �?
    ...
    �?�?TryOperation和TryCancel应该具备良好的性能特�?                             �?
    �?                                                                              �?
    ╚═══════════════════════════════════════════════════════════════════════════════�?
