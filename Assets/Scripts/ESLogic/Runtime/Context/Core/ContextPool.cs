...
namespace ES
{
    ...
    public class ContextPool 
    {
        ...
        {
            ClearNonPersistentRuntimeValues();
        }

        public void ClearNonPersistentRuntimeValues()
        {
            if (_ContextValues == null || _ContextValues.Count == 0)
                return;

            _runtimeRemoveKeys ??= new List<string>(8);
            _runtimeRemoveKeys.Clear();
            foreach (var pair in _ContextValues)
            {
                if (pair.Value == null || pair.Value.IsPersistent)
                    continue;

                _runtimeRemoveKeys.Add(pair.Key);
            }

            if (_runtimeRemoveKeys.Count == 0)
                return;

            for (int i = 0; i < _runtimeRemoveKeys.Count; i++)
            {
                string key = _runtimeRemoveKeys[i];
                if (!_ContextValues.TryGetValue(key, out var value))
                    continue;

                if (enabled)
                    value.RemoveReceivePool(this);

                _ContextValues.Remove(key);
                value.TryAutoPushedToPool();
            }
        ...
        public void TryAddNewContextValueBySplits(EnumCollect.ContextValueType Contextitecture, string key, object value, bool send = false, bool persistent = true)
        {
            ...
            {
                var newOne = IContextitectureValue.Create(Contextitecture, key, send, persistent);
        ...
        public void TryAddNewContextValueFromContextValueCopy(IContextitectureValue arValue)
        {
            ...
            {
                var value = IContextitectureValue.Create(arValue.ContextType, arValue.TheKey, arValue.WillSendLink, arValue.IsPersistent);
                value.DeepCloneFrom(arValue);
                AddContextValueTruelyToCreate(value.TheKey, value);
            }
        }
        #endregion
...
#pragma warning restore CS0414 
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining),  ]
        public void SetBool(string key, bool use, EnumCollect.HandleTwoBool function = EnumCollect.HandleTwoBool.Set, bool persistentIfCreate = true)
        {
            ...
            {
                var value = IContextitectureValue.Create(EnumCollect.ContextValueType.BoolValue, key, false, persistentIfCreate);
                value.SetBool(ESDesignUtility.Function.HandleTwoBool(true, use, function));
                AddContextValueTruelyToCreate(key, value);
            }
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        [TabGroup("内置通用操作测试"),Button("操作浮点值"),  ]
        public void SetFloat(string key, float use, EnumCollect.HandleTwoNumber function = EnumCollect.HandleTwoNumber.Set, bool persistentIfCreate = true)
        {
            if (_ContextValues.TryGetValue(key, out var vv))
            {
            ...
            {
                var value = IContextitectureValue.Create(EnumCollect.ContextValueType.FloatValue, key, false, persistentIfCreate);
                value.SetFloat(ESDesignUtility.Function.HandleTwoFloat(0, use, function));
                AddContextValueTruelyToCreate(key, value);
            }
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        [TabGroup("内置通用操作测试"), Button("操作整数值"),  ]
        public void SetInt(string key, int use, EnumCollect.HandleTwoNumber function = EnumCollect.HandleTwoNumber.Set, bool persistentIfCreate = true)
        {
            if (_ContextValues.TryGetValue(key, out var vv))
            {
            ...
            {
                var value = IContextitectureValue.Create(EnumCollect.ContextValueType.IntValue, key, false, persistentIfCreate);
                value.SetInt(ESDesignUtility.Function.HandleTwoInt(0, use, function));
                AddContextValueTruelyToCreate(key, value);
            }
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        [TabGroup("内置通用操作测试"), Button("直接设置浮点值"),  ]
        public void SetFloatDirect(string key, float use, bool EnableSendLinkIfCreateNew = false, bool persistentIfCreate = true)
        {
            if (_ContextValues.TryGetValue(key, out var vv))
            {
                vv.SetFloat(use);
            ...
            {
                TryAddNewContextValueBySplits(EnumCollect.ContextValueType.FloatValue, key, use, EnableSendLinkIfCreateNew, persistentIfCreate);
            }
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        [TabGroup("内置通用操作测试"), Button("直接设置向量"),  ]
        public void SetVectorDirect(string key, Vector3 use, bool EnableSendLinkIfCreateNew = false, bool persistentIfCreate = true)
        {
            if (_ContextValues.TryGetValue(key, out var vv))
            {
                vv.SetVector(use);
            ...
            {
                TryAddNewContextValueBySplits(EnumCollect.ContextValueType.VectorValue, key, use, EnableSendLinkIfCreateNew, persistentIfCreate);
            }
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        [TabGroup("内置通用操作测试"), Button("直接设置整数值"),  ]
        public void SetIntDirect(string key, int use, bool EnableSendLinkIfCreateNew = false, bool persistentIfCreate = true)
        {
            if (_ContextValues.TryGetValue(key, out var vv))
            {
                vv.SetInt(use);
            ...
            {
                TryAddNewContextValueBySplits(EnumCollect.ContextValueType.IntValue, key, use, EnableSendLinkIfCreateNew, persistentIfCreate);
            }
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        [TabGroup("内置通用操作测试"), Button("直接设置布尔值"),  ]
        public void SetBoolDirect(string key, bool use, bool EnableSendLinkIfCreateNew = false, bool persistentIfCreate = true)
        {
            if (_ContextValues.TryGetValue(key, out var vv))
            {
                vv.SetBool(use);
            ...
            {
                TryAddNewContextValueBySplits(EnumCollect.ContextValueType.BoolValue, key, use, EnableSendLinkIfCreateNew, persistentIfCreate);
            }
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        [TabGroup("内置通用操作测试"), Button("直接设置浮点数"),  ]
        public void SetStringDirect(string key, string use, bool EnableSendLinkIfCreateNew = false, bool persistentIfCreate = true)
        {
            if (_ContextValues.TryGetValue(key, out var vv))
            {
                vv.SetString(use);
            ...
            {
                TryAddNewContextValueBySplits(EnumCollect.ContextValueType.StringValue, key, use, EnableSendLinkIfCreateNew, persistentIfCreate);
            }
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        public void SetFloatQuick_Add(string key, float num, bool persistentIfCreate = true)
        {
            if (_ContextValues.TryGetValue(key, out var vv))
            {
        ...
        public void SetTagQuick_Use(string key, bool persistentIfCreate = true)
        {
            if (_ContextValues.TryGetValue(key, out var vv))
            {
            ...
            {
                TryAddNewContextValueBySplits(EnumCollect.ContextValueType.DynamicTag, key, 5, false, persistentIfCreate);
            }
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        [TabGroup("内置快捷操作测试"), Button("标签设置为禁用"),  ]
        public void SetTagQuick_CancelUse(string key, bool persistentIfCreate = true)
        {
            if (_ContextValues.TryGetValue(key, out var vv))
            {
        ...
        public void SetTagQuick_UseableTime(string key, float f, bool persistentIfCreate = true)
        {
            if (_ContextValues.TryGetValue(key, out var vv))
            {
            ...
            {
                TryAddNewContextValueBySplits(EnumCollect.ContextValueType.DynamicTag, key, f, false, persistentIfCreate);
            }
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        [TabGroup("内置快捷操作测试"), Button("设置标签可用并设置有效时间"),  ]
        public void SetTagQuick_SetUseableAndEnable(string key, float f, bool persistentIfCreate = true)
        {
            if (_ContextValues.TryGetValue(key, out var vv))
            {
                vv.SetFloat(f);//对于Tag来说--就是重制时间
        ...
        public void SetBoolFunc(string key, Func<bool, bool> func)
        {
            ...
            {
                var value = IContextitectureValue.Create(EnumCollect.ContextValueType.BoolValue, key, false, true);
                value.SetBool((func != null) ? func.Invoke(false) : false);
                AddContextValueTruelyToCreate(key, value);
            }
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void SetIntFunc(string key, Func<int, int> func)
        {
            ...
            {
                var value = IContextitectureValue.Create(EnumCollect.ContextValueType.IntValue, key, false, true);
                value.SetInt((func != null) ? func.Invoke(0) : 0);
                AddContextValueTruelyToCreate(key, value);
            }
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void SetFloatFunc(string key, Func<float, float> func)
        {
            ...
            {
                var value = IContextitectureValue.Create(EnumCollect.ContextValueType.FloatValue, key, false, true);
                value.SetFloat((func != null) ? func.Invoke(0f) : 0f);
                AddContextValueTruelyToCreate(key, value);
            }
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void SetStringFunc(string key, Func<string, string> func)
        {
            ...
            {
                var value = IContextitectureValue.Create(EnumCollect.ContextValueType.StringValue, key, false, true);
                value.SetString((func != null) ? func.Invoke("") : "");
                AddContextValueTruelyToCreate(key, value);
            }
        }
        #endregion
        ...
        public void RemoveContextValueTruelyToCreate(string key, IContextitectureValue Contextitecture)
        {
            ...
            {
                if (enabled)
                    Context.RemoveReceivePool(this);

                _ContextValues.Remove(key);
                Context.TryAutoPushedToPool();
            }
        }
        #endregion
    }
