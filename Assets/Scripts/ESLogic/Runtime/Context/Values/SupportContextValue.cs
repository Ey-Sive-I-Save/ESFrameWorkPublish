...
namespace ES
{
    ...
    public class ContextitectureTypeValue_DynamicTag : ContextitectureValue<float>
    {
        ...
        protected override void ResetPoolOnlyState()
        {
            MaxUseableDURA = 999999;
        }
        //Value=>获得时间
    ...
    public abstract class ContextitectureValue<ValueT> : IContextitectureValue
    {
        ...
        [NonSerialized] private bool poolOwned;
        [NonSerialized] private bool isRecycled;
        public bool IsRecycled { get => isRecycled; set => isRecycled = value; }
        public bool IsPoolOwned => poolOwned;
        #endregion
        ...
        [LabelText("持久条目"), Tooltip("开启后不会被 OpSupport 的单次激活退出清理。检查器初始化值默认开启；Op 临时写入通常关闭。")]
        ...
        public bool IsPersistent { get => Persistent; set => Persistent = value; }

        internal void PrepareFromPool(string useKey, bool send, bool persistent)
        {
            key = useKey;
            _value = default;
            SendLink = send;
        ...
        protected virtual void ResetPoolOnlyState()
        {
        }
        ...
        {
            if (IsRecycled || !poolOwned)
                return;

            if (!ESContextitectureValuePools.Release(this))
            {
                OnResetAsPoolable();
                IsRecycled = true;
            }
    ...
    internal static class ESContextitectureValuePools
    {
        private const int InitCount = 16;
        private const int MaxCount = 4096;

        private static readonly ESSimplePool<ContextitectureTypeValue_ClassT> ClassTPool = new ESSimplePool<ContextitectureTypeValue_ClassT>(
            factoryMethod: () => new ContextitectureTypeValue_ClassT(),
            initCount: InitCount,
            maxCount: MaxCount,
            poolDisplayName: "Context Value ClassT Pool",
            groupName: "Context");

        private static readonly ESSimplePool<ContextitectureTypeValue_UnityObject> UnityObjectPool = new ESSimplePool<ContextitectureTypeValue_UnityObject>(
            factoryMethod: () => new ContextitectureTypeValue_UnityObject(),
            initCount: InitCount,
            maxCount: MaxCount,
            poolDisplayName: "Context Value UnityObject Pool",
            groupName: "Context");

        private static readonly ESSimplePool<ContextitectureTypeValue_DynamicTag> DynamicTagPool = new ESSimplePool<ContextitectureTypeValue_DynamicTag>(
            factoryMethod: () => new ContextitectureTypeValue_DynamicTag(),
            initCount: InitCount,
            maxCount: MaxCount,
            poolDisplayName: "Context Value DynamicTag Pool",
            groupName: "Context");

        private static readonly ESSimplePool<ContextitectureTypeValue_Float> FloatPool = new ESSimplePool<ContextitectureTypeValue_Float>(
            factoryMethod: () => new ContextitectureTypeValue_Float(),
            initCount: InitCount,
            maxCount: MaxCount,
            poolDisplayName: "Context Value Float Pool",
            groupName: "Context");

        private static readonly ESSimplePool<ContextitectureTypeValue_Vector> VectorPool = new ESSimplePool<ContextitectureTypeValue_Vector>(
            factoryMethod: () => new ContextitectureTypeValue_Vector(),
            initCount: InitCount,
            maxCount: MaxCount,
            poolDisplayName: "Context Value Vector Pool",
            groupName: "Context");

        private static readonly ESSimplePool<ContextitectureTypeValue_Int> IntPool = new ESSimplePool<ContextitectureTypeValue_Int>(
            factoryMethod: () => new ContextitectureTypeValue_Int(),
            initCount: InitCount,
            maxCount: MaxCount,
            poolDisplayName: "Context Value Int Pool",
            groupName: "Context");

        private static readonly ESSimplePool<ContextitectureTypeValue_String> StringPool = new ESSimplePool<ContextitectureTypeValue_String>(
            factoryMethod: () => new ContextitectureTypeValue_String(),
            initCount: InitCount,
            maxCount: MaxCount,
            poolDisplayName: "Context Value String Pool",
            groupName: "Context");

        private static readonly ESSimplePool<ContextitectureTypeValue_Bool> BoolPool = new ESSimplePool<ContextitectureTypeValue_Bool>(
            factoryMethod: () => new ContextitectureTypeValue_Bool(),
            initCount: InitCount,
            maxCount: MaxCount,
            poolDisplayName: "Context Value Bool Pool",
            groupName: "Context");

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IContextitectureValue Rent(EnumCollect.ContextValueType type, string key, bool send, bool persistent)
        {
            switch (type)
            {
                case EnumCollect.ContextValueType.ClassTValue:
                {
                    var value = ClassTPool.GetInPool();
                    value.PrepareFromPool(key, send, persistent);
                    return value;
                }
                case EnumCollect.ContextValueType.UnityObjectTValue:
                {
                    var value = UnityObjectPool.GetInPool();
                    value.PrepareFromPool(key, send, persistent);
                    return value;
                }
                case EnumCollect.ContextValueType.DynamicTag:
                {
                    var value = DynamicTagPool.GetInPool();
                    value.PrepareFromPool(key, send, persistent);
                    return value;
                }
                case EnumCollect.ContextValueType.FloatValue:
                {
                    var value = FloatPool.GetInPool();
                    value.PrepareFromPool(key, send, persistent);
                    return value;
                }
                case EnumCollect.ContextValueType.IntValue:
                {
                    var value = IntPool.GetInPool();
                    value.PrepareFromPool(key, send, persistent);
                    return value;
                }
                case EnumCollect.ContextValueType.BoolValue:
                {
                    var value = BoolPool.GetInPool();
                    value.PrepareFromPool(key, send, persistent);
                    return value;
                }
                case EnumCollect.ContextValueType.StringValue:
                {
                    var value = StringPool.GetInPool();
                    value.PrepareFromPool(key, send, persistent);
                    return value;
                }
                case EnumCollect.ContextValueType.VectorValue:
                {
                    var value = VectorPool.GetInPool();
                    value.PrepareFromPool(key, send, persistent);
                    return value;
                }
                default:
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool Release(IContextitectureValue value)
        {
            switch (value)
            {
                case ContextitectureTypeValue_ClassT typed: return ClassTPool.PushToPool(typed);
                case ContextitectureTypeValue_UnityObject typed: return UnityObjectPool.PushToPool(typed);
                case ContextitectureTypeValue_DynamicTag typed: return DynamicTagPool.PushToPool(typed);
                case ContextitectureTypeValue_Float typed: return FloatPool.PushToPool(typed);
                case ContextitectureTypeValue_Vector typed: return VectorPool.PushToPool(typed);
                case ContextitectureTypeValue_Int typed: return IntPool.PushToPool(typed);
                case ContextitectureTypeValue_String typed: return StringPool.PushToPool(typed);
                case ContextitectureTypeValue_Bool typed: return BoolPool.PushToPool(typed);
                default: return false;
            }
        }
    }

    public interface IContextitectureValue : IDeepClone<IContextitectureValue>,Sirenix.OdinInspector.ISearchFilterable, IPoolableAuto
    {
        
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IContextitectureValue Create(EnumCollect.ContextValueType type, string key, bool send)
        {
            return Create(type, key, send, true);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IContextitectureValue Create(EnumCollect.ContextValueType type, string key, bool send, bool persistent)
        {
            return ESContextitectureValuePools.Rent(type, key, send, persistent);
        }
