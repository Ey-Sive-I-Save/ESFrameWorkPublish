...
namespace ES
{
    ...
    public class GameCoreEditorGlobalData : ESEditorGlobalSo<GameCoreEditorGlobalData>
    {
        ...
        [InfoBox("在这里声明 EnumKey/StringKey、HotSlot/Sparse、Entity Usage 与稳定传输范围。BakeTable、RuntimeKey、SchemaHash 均由此列表生成，禁止手改 BakeTable。")]
        [ListDrawerSettings(DefaultExpandedState = false)]
    ...
    public static class GameCoreDefaultRules
    {
        ...
        public static List<GameCoreAICommandTemplate> CreateAICommandTemplates()
        {
            return new List<GameCoreAICommandTemplate>
            {
                ...
                Command("新增或调整GameTag", "Tag", "请按 GameCoreEditorGlobalData 的 GameTag 规范处理标签：\n1. 先判断它是否适合表达可组合的运行时事实或条件；阵营关系、身份、数值、资源定位和状态机执行仍归各自领域。\n2. 在 GameCoreTagDefinition 中声明稳定身份（EnumKey、StringKey 或两者绑定）、StorageTier（HotSlot/Sparse）、Availability 与 SaveGame/Network 传输范围；Bake 后才生成 RuntimeKey。绝不重排已有 Enum 编号，RuntimeKey 仅当前 Schema 进程可用，禁止写入配置、存档或网络。\n3. 所有策划条件统一使用 ESTagConditionConfig 的 required、requiredAny、forbidden；从统一 Picker 选择 ESTagStableReference。业务调用 ESTagCollection.Matches(config) 或 TryMatches(config, out matches, out error)，禁止裸 RuntimeKey、手动编译和按 Enum/String 拆分字段。\n4. 所有状态写入者统一使用 ESTagGrantConfig.tags + ESTagLeaseSet：Buff、装备、状态环境、区域、任务等在自己的激活路径申请 Lease，在切换、退出、禁用、销毁路径 Dispose。禁止直接减计数或按 Tag 强制移除。Buff 只能授予 RuntimeFact 主 Enum Tag；能力入口仍由对应组件维护。\n5. Entity 只是 ESTagCollection 的一个 Host；其他需要被查询的对象也可持有通用容器。Lease 来源仅用于其生命周期与可选诊断，不参与 Tag 身份、存储和查询。\n6. Skill、State、AI、Interaction 配置接入 ESTagConditionConfig；HitResolver 只调用 ESHitTagEligibility 决定命中资格，物理候选、伤害、阵营、友伤、部位倍率仍归各自领域。\n7. 存档/联机只传 ESTagStableSnapshot 的稳定引用和 SchemaHash。先在 Catalog 的 stableTransferScopes 声明 SaveGame/Network，再以明确 LeaseSet 恢复；禁止传 RuntimeKey、Count、HotMask 或 Source。\n8. 控制玩家需要输入/UI 投影时，只能由 ESGameManager.LocalControl 声明/切换本地控制实体，禁止任意 Entity 自行创建 Projector。\n9. 运行“验证GameTag规则”“验证全部Buff的GameTag配置”、稳定 Key 审计和核心自检；说明写入者、消费者、Snapshot Scope 与验证结果。\n需求：<在这里补充标签用途>"),
