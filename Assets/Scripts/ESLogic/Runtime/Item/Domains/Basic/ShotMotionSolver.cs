    ...
    public static class ShotMotionSolver
    {
            ...
            float deltaTime)
        {
            ...
            if (state.elapsedTime < Mathf.Max(0f, config.launchDelay))
                return BuildStaticResult(state, ShotMotionKind.Delayed);

            if (state.elapsedTime < Mathf.Max(0f, config.launchDelay) + Mathf.Max(0f, config.warmupTime))
                return BuildStaticResult(state, ShotMotionKind.Warmup);
            ...
            if ((config.flags & ShotMotionFlags.UseGravity) != 0)
                state.velocity += config.gravity * deltaTime;

            if ((config.flags & ShotMotionFlags.UseDrag) != 0 && config.drag > 0f)
                state.velocity *= 1f / (1f + config.drag * deltaTime);

            if ((config.flags & ShotMotionFlags.ClampSpeed) != 0 && config.maxSpeed > 0f)
            ...
            ShotMotionKind kind = ResolveKind(state, config, remainingDistance);

            return new ShotMotionResult
            {
                kind = kind,
        ...
        private static Vector3 ResolveDesiredVelocity(in ShotMotionState state, in ShotMotionConfig config)
        {
        ...
        private static ShotMotionResult BuildStaticResult(in ShotMotionState state, ShotMotionKind kind)
        {
            return new ShotMotionResult
            {
                kind = kind,
        ...
        private static bool IsTrackingEnabled(float elapsedTime, in ShotMotionConfig config)
        {
        ...
        private static float ResolveRemainingDistance(in ShotMotionState state)
        {
            if (!state.hasTarget)
        ...
        private static ShotMotionKind ResolveKind(in ShotMotionState state, in ShotMotionConfig config, float remainingDistance)
        {
            if (config.maxLifetime > 0f && state.elapsedTime >= config.maxLifetime)
                return ShotMotionKind.Expired;

            if (state.hasTarget && remainingDistance <= Mathf.Max(0f, config.arriveDistance))
                return ShotMotionKind.Arrived;

            return ShotMotionKind.Moving;
        }
    }
}
