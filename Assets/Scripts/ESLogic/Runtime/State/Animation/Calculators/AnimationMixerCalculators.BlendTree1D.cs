    ...
    // ==================== 1D混合树 ====================
        ...
        public class StateAnimationMixCalculatorForBlendTree1D : StateAnimationMixCalculator
        {
            ...
            protected override bool InitializeRuntimeInternal(AnimationCalculatorRuntime runtime, PlayableGraph graph, ref Playable output)
            {
                ...
                {
                    if (samples[i] == null || samples[i].clip == null)
                    {
                        StateMachineDebugSettings.Instance.LogError($"[BlendTree1D] 采样点{i}缺少Clip，已拒绝初始化，避免权重输出到空输入");
                        return false;
                    }
                }
                ...
                if (samples.Length == 1)
                {
                    ...
                    runtime.RegisterClipOverrideSlot(0, samples[0].clip, samples[0].marker);
                    runtime.singlePlayable = AnimationClipPlayable.Create(graph, samples[0].clip);
                    runtime.weightCache = new float[1] { 1f };
                    runtime.weightTargetCache = new float[1] { 1f };
                    runtime.weightVelocityCache = new float[1];
                ...
                runtime.EnsureMixerInputWeightCache(samples.Length);
                runtime.EnsureClipOverrideSlots(samples.Length);
                ...
                for (int i = 0; i < samples.Length; i++)
                {
                    if (samples[i].clip != null)
                    {
                        ...
                        runtime.RegisterClipOverrideSlot(i, samples[i].clip, samples[i].marker);
                        graph.Connect(runtime.playables[i], 0, runtime.mixer, i);
                        runtime.SetMixerInputWeightIfChanged(runtime.mixer, i, 0f);
            ...
            private void CalculateWeights1D(AnimationCalculatorRuntime runtime, float input, float deltaTime)
            {
                ...
                else
                {
                    ...
                    float range = rightThreshold - leftThreshold;
                    float t = Mathf.Abs(range) > 0.0001f ? (input - leftThreshold) / range : 0f;
            ...
            public override void ResetRuntimeForReuse(AnimationCalculatorRuntime runtime)
            {
                ...
                if (runtime == null || !runtime.mixer.IsValid() || samples == null || samples.Length == 0)
                    return;

                runtime.lastInput = 0f;
                ...
                if (samples.Length == 1)
                {
                    ...
                    {
                        runtime.singlePlayable.SetTime(0d);
                        runtime.singlePlayable.SetSpeed(1d);
                    }

                    if (runtime.weightCache != null && runtime.weightCache.Length > 0)
                        runtime.weightCache[0] = 1f;
                    if (runtime.weightTargetCache != null && runtime.weightTargetCache.Length > 0)
                        runtime.weightTargetCache[0] = 1f;
                    if (runtime.weightVelocityCache != null && runtime.weightVelocityCache.Length > 0)
                        runtime.weightVelocityCache[0] = 0f;
                ...
                for (int i = 0; i < count; i++)
                {
                    if (runtime.playables != null && i < runtime.playables.Length && runtime.playables[i].IsValid())
                    {
                        runtime.playables[i].SetTime(0d);
                        runtime.playables[i].SetSpeed(1d);
                    }

                    if (runtime.weightCache != null && i < runtime.weightCache.Length)
                        runtime.weightCache[i] = 0f;
                    if (runtime.weightTargetCache != null && i < runtime.weightTargetCache.Length)
                        runtime.weightTargetCache[i] = 0f;
                    if (runtime.weightVelocityCache != null && i < runtime.weightVelocityCache.Length)
            ...
            public override void ImmediateUpdate(AnimationCalculatorRuntime runtime, in StateMachineContext context)
            {
                ...
                else
                {
                    ...
                    float rightThreshold = samples[rightIndex].threshold;
                    float range = rightThreshold - leftThreshold;
                    float t = Mathf.Abs(range) > 0.0001f ? (mappedInput - leftThreshold) / range : 0f;
                    runtime.weightTargetCache[leftIndex] = 1f - t;
                    runtime.weightTargetCache[rightIndex] = t;
                }
            ...
            public override AnimationClip GetCurrentClip(AnimationCalculatorRuntime runtime)
            {
                ...
                {
                    if (!runtime.playables[i].IsValid())
                        continue;
                ...
                return runtime.playables[maxWeightIndex].IsValid()
                    ? runtime.playables[maxWeightIndex].GetAnimationClip()
                    : null;
            }
            ...
            public override bool OverrideClip(AnimationCalculatorRuntime runtime, int clipIndex, AnimationClip newClip)
            {
                if (samples.Length == 1 && clipIndex == 0 && runtime.singlePlayable.IsValid())
                {
                    ...
                    if (runtime.mixer.IsValid())
                    {
                        singleGraph.Connect(runtime.singlePlayable, 0, runtime.mixer, 0);
                        runtime.SetMixerInputWeightIfChanged(runtime.mixer, 0, 1f);
                    }
            ...
            {
                if (sourceClip == null || runtime == null)
                    return false;
            ...
            {
                if (string.IsNullOrWhiteSpace(marker) || samples == null)
                    return false;
                ...
                for (int i = 0; i < samples.Length; i++)
                {
                    if (samples[i] != null && string.Equals(samples[i].marker, marker, StringComparison.OrdinalIgnoreCase))
                        changed |= OverrideClip(runtime, i, newClip);
                }
