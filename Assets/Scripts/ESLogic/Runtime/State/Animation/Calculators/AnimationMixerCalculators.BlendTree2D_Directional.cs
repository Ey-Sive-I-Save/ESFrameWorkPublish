        ...
        public abstract class StateAnimationMixCalculatorForBlendTree2D : StateAnimationMixCalculator
        {
            ...
            protected override bool InitializeRuntimeInternal(AnimationCalculatorRuntime runtime, PlayableGraph graph, ref Playable output)
            {
                ...
                for (int i = 0; i < samples.Length; i++)
                {
                    ...
                    {
                        debugSettings.LogError($"[BlendTree2D] 采样点{i}缺少Clip，已拒绝初始化，避免权重输出到空输入");
                        return false;
                    }
                }
                ...
                for (int i = 0; i < samples.Length; i++)
                {
                    if (samples[i].clip != null)
                    {
                        ...
                        runtime.SetMixerInputWeightIfChanged(runtime.mixer, i, initialWeight);
            ...
            public override void ResetRuntimeForReuse(AnimationCalculatorRuntime runtime)
            {
                ...
                runtime.lastInput2D = Vector2.zero;
                ...
                for (int i = 0; i < samples.Length; i++)
                {
                    ...
                    float weight = i == centerIndex ? 1f : 0f;
