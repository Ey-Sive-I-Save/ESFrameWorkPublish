...
namespace ES
{
    ...
    { 
        ...
        private static void InitPhaseRuntime(AnimationCalculatorRuntime runtime, PlayableGraph graph, int index, Phase4Config phase)
        {
            ...
            if (phase.primaryClip != null)
            {
                if (phase.enableSecondaryClipBlend && phase.secondaryClip != null)
                {
                    ...
                    runtime.RegisterClipOverrideSlot(index * 2, phase.primaryClip, StateAnimationMixCalculatorForPhase4.ResolvePhaseClipMarker(phase.primaryClipMarker, phase.phaseName, index, "Primary"));
                    ...
                    runtime.RegisterClipOverrideSlot(index * 2 + 1, phase.secondaryClip, StateAnimationMixCalculatorForPhase4.ResolvePhaseClipMarker(phase.secondaryClipMarker, phase.phaseName, index, "Secondary"));
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void RestartPhasePlayables(AnimationCalculatorRuntime runtime, int phaseIndex)
        {
            ...
            if (runtime.phaseUsesCalculator != null && phaseIndex < runtime.phaseUsesCalculator.Length && runtime.phaseUsesCalculator[phaseIndex])
            {
                if (runtime.phaseRuntimes != null && phaseIndex < runtime.phaseRuntimes.Length)
                {
                    ...
                    if (pr != null && phase != null && phase.phaseCalculator != null)
                    {
                        phase.phaseCalculator.ResetRuntimeForReuse(pr);
                    }
                    else if (pr != null && pr.outputPlayable.IsValid())
                    {
                        pr.outputPlayable.SetTime(0d);
                    }
                }
        ...
        private static string ResolvePhaseClipMarker(string explicitMarker, string phaseName, int index, string localName)
        {
            return !string.IsNullOrWhiteSpace(explicitMarker)
                ? explicitMarker
                : BuildPhaseMarker(phaseName, index, localName);
        }
