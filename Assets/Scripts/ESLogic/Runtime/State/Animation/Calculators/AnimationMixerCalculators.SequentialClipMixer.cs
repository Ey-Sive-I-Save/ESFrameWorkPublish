...
namespace ES
{
        ...
        public class SequentialClipMixer : StateAnimationMixCalculator
        {
            ...
            public override void ResetRuntimeForReuse(AnimationCalculatorRuntime runtime)
            {
                ...
                runtime.InvalidateMixerInputWeightCache();

                if (runtime.playables != null)
                {
                    ResetPhasePlayable(runtime, 0, entrySpeed, runtime.sequencePhase == 0);
                    ResetPhasePlayable(runtime, 1, mainConfig != null ? mainConfig.speed : 1f, runtime.sequencePhase == 1);
                    ResetPhasePlayable(runtime, 2, exitSpeed, runtime.sequencePhase == 2);
                }

                UpdatePhaseWeights(runtime);
            }

            private static void ResetPhasePlayable(AnimationCalculatorRuntime runtime, int index, float speed, bool active)
            {
                if (runtime == null || runtime.playables == null || (uint)index >= (uint)runtime.playables.Length)
                    return;

                var playable = runtime.playables[index];
                if (!playable.IsValid())
                    return;
