...
namespace ES
{
    ...
    public class StateAnimationMixCalculatorForSequentialStates : StateAnimationMixCalculator
    {
        ...
        public class SequentialPhase
        {
            ...
            [LabelText("主动画标记")]
            [Tooltip("用于运行时按标记替换主动画。留空时使用阶段名自动生成。")]
            [ShowIf("@phaseCalculator == null && primaryClip != null")]
            public string primaryClipMarker;
            ...
            [LabelText("混合动画标记")]
            [Tooltip("用于运行时按标记替换混合动画。仅在启用主/混合动画混合时生效；留空时使用阶段名自动生成。")]
            [ShowIf("@phaseCalculator == null && enableSecondaryClipBlend && secondaryClip != null")]
            public string secondaryClipMarker;
        ...
        protected override bool InitializeRuntimeInternal(AnimationCalculatorRuntime runtime, PlayableGraph graph, ref Playable output)
        {
                ...
                else
                {
                    ...
                    if (phase.primaryClip != null)
                    {
                        if (phase.enableSecondaryClipBlend && phase.secondaryClip != null)
                        {
                            ...
                            runtime.RegisterClipOverrideSlot(i * 2, phase.primaryClip, ResolvePhaseClipMarker(phase.primaryClipMarker, phase.phaseName, i, "Primary"));
                            ...
                            runtime.RegisterClipOverrideSlot(i * 2 + 1, phase.secondaryClip, ResolvePhaseClipMarker(phase.secondaryClipMarker, phase.phaseName, i, "Secondary"));
            ...
            UpdatePhasePlayableSpeeds(runtime, 0, -1);
        ...
        private static string ResolvePhaseClipMarker(string explicitMarker, string phaseName, int phaseIndex, string localName)
        {
            return !string.IsNullOrWhiteSpace(explicitMarker)
                ? explicitMarker
                : BuildPhaseMarker(phaseName, phaseIndex, localName);
        }
        ...
        public override void UpdateWeights(AnimationCalculatorRuntime runtime, in StateMachineContext context, float deltaTime)
        {
            ...
            if (shouldTransition)
            {
                ...
                if (currentPhase != prevPhase)
                    RestartPhasePlayable(runtime, currentPhase);
            ...
            UpdatePhasePlayableSpeeds(runtime, currentPhase, prevPhaseIndex);
        ...
        public override void ImmediateUpdate(AnimationCalculatorRuntime runtime, in StateMachineContext context)
        {
            ...
            // 不推进时间，只根据当前阶段索引设置权重
            int currentPhase = runtime.sequencePhaseIndex;
            UpdatePhasePlayableSpeeds(runtime, currentPhase, -1);
            for (int i = 0; i < phases.Length; i++)
            {
        ...
        private void RestartPhasePlayable(AnimationCalculatorRuntime runtime, int phaseIndex)
        {
            if (runtime == null || phases == null || phaseIndex < 0 || phaseIndex >= phases.Length)
                return;
            ...
            {
                if (phase.phaseCalculator != null && runtime.phaseRuntimes != null && phaseIndex < runtime.phaseRuntimes.Length)
                {
                    var phaseRuntime = runtime.phaseRuntimes[phaseIndex];
                    if (phaseRuntime != null)
                        phase.phaseCalculator.ResetRuntimeForReuse(phaseRuntime);
                }
        ...
        private static void SetPhasePlayableSpeed(AnimationCalculatorRuntime runtime, int phaseIndex, double speed)
        {
            if (runtime == null || phaseIndex < 0)
                return;
            ...
            if (runtime.phaseUsesCalculator != null && phaseIndex < runtime.phaseUsesCalculator.Length && runtime.phaseUsesCalculator[phaseIndex])
            {
                ...
                {
                    var phaseRuntime = runtime.phaseRuntimes[phaseIndex];
                    if (phaseRuntime != null && phaseRuntime.outputPlayable.IsValid())
                        phaseRuntime.outputPlayable.SetSpeed(speed);
                }

                if (runtime.phaseOutputs != null && phaseIndex < runtime.phaseOutputs.Length && runtime.phaseOutputs[phaseIndex].IsValid())
                    runtime.phaseOutputs[phaseIndex].SetSpeed(speed);
                return;
            }
        ...
        private void UpdatePhasePlayableSpeeds(AnimationCalculatorRuntime runtime, int currentPhase, int prevPhaseIndex)
        {
            if (runtime == null || phases == null)
                return;

            for (int i = 0; i < phases.Length; i++)
            {
                bool active = i == currentPhase || (prevPhaseIndex >= 0 && i == prevPhaseIndex);
                SetPhasePlayableSpeed(runtime, i, active ? 1d : 0d);
            }
        }
        ...
        public override void ResetRuntimeForReuse(AnimationCalculatorRuntime runtime)
        {
            ...
            runtime.RewindCachedPlayableTreeForEnter();
            runtime.ResumeCachedPlayableTree();
            ResetSequence(runtime);

            if (runtime.mixer.IsValid() && phases != null)
            {
                for (int i = 0; i < phases.Length; i++)
                {
                    runtime.mixer.SetInputWeight(i, i == 0 ? 1f : 0f);
                }
            }
            ...
            RestartPhasePlayable(runtime, 0);
            UpdatePhasePlayableSpeeds(runtime, 0, -1);
        }
