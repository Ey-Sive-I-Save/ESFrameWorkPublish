...
namespace ES
{
    ...
    public class AnimationCalculatorRuntime : IPoolableAuto
    {
        ...
        /// <summary>技能时间线 Idle 输入权重缓存，避免每帧重复 SetInputWeight</summary>
        public float skillTimelineIdleWeightCache;
        public bool skillTimelineIdleWeightCacheValid;
        ...
        private void ResetDataOnly()
        {
            ...
            skillTimelineIdleWeightCache = 0f;
            skillTimelineIdleWeightCacheValid = false;
        ...
        public void PauseCachedPlayableTree()
        {
            if (outputPlayable.IsValid())
                outputPlayable.SetSpeed(0d);
        }

        public void ResumeCachedPlayableTree()
        {
            if (outputPlayable.IsValid())
                outputPlayable.SetSpeed(1d);
        }

        public void InvalidateMixerInputWeightCache()
        {
        ...
        {
            InvalidateMixerInputWeightCache();
            lastInput = 0f;
            inputVelocity = 0f;
            lastInput2D = Vector2.zero;
            inputVelocity2D = Vector2.zero;
            ...
            skillTimelineIdleWeightCache = 0f;
            skillTimelineIdleWeightCacheValid = false;
            ResetFloatArray(weightVelocityCache);
            ResetFloatArray(weightVelocities);

            RewindPlayable(outputPlayable);
            RewindPlayable(mixer);
            RewindPlayable(singlePlayable);
            RewindPlayableArray(playables);
            RewindPlayableArray(phaseOutputs);
            RewindPlayableArray(phaseMixers);
            RewindPlayableArray(phasePrimaryPlayables);
            RewindPlayableArray(phaseSecondaryPlayables);

            childRuntime?.RewindCachedPlayableTreeForEnter();
            childRuntimeB?.RewindCachedPlayableTreeForEnter();

            if (phaseRuntimes != null)
            {
                for (int i = 0; i < phaseRuntimes.Length; i++)
                {
                    phaseRuntimes[i]?.RewindCachedPlayableTreeForEnter();
                }
            }
        }

        private static void RewindPlayable(Playable playable)
        {
            if (!playable.IsValid())
                return;
        ...
        private static void RewindPlayable(AnimationMixerPlayable playable)
        {
            if (!playable.IsValid())
                return;
        ...
        private static void RewindPlayable(AnimationClipPlayable playable)
        {
            if (!playable.IsValid())
                return;
        ...
        private static void ResetFloatArray(float[] values)
        {
            if (values == null)
                return;
        ...
        private static void RewindPlayableArray(Playable[] playables)
        {
            if (playables == null)
                return;
            ...
            for (int i = 0; i < playables.Length; i++)
            {
                RewindPlayable(playables[i]);
            }
        }

        private static void RewindPlayableArray(AnimationMixerPlayable[] playables)
        {
            if (playables == null)
                return;
        ...
        private static void RewindPlayableArray(AnimationClipPlayable[] playables)
        {
            if (playables == null)
                return;
