...
namespace ES
{
    public partial class StateBase
    {
        ...
        internal void SetPlayableWeight(float weight)
        {
            ...
            {
                LogAnimationContractWarning($"SetPlayableWeight: weight={weight} 超出[0,1]，已自动Clamp。State={GetStateNameSafe()}");
        ...
        internal void SetPlayableWeightAssumeBound(float weight)
        {
            ...
            if (weight < 0f || weight > 1f)
            {
                LogAnimationContractWarning($"SetPlayableWeightAssumeBound: weight={weight} 超出[0,1]，已自动Clamp。State={GetStateNameSafe()}");
                weight = Mathf.Clamp01(weight);
            }

            if (_layerRuntime == null)
            {
                LogAnimationContractWarning($"SetPlayableWeightAssumeBound: _layerRuntime 为空，降级为 SetPlayableWeight。State={GetStateNameSafe()}");
                SetPlayableWeight(weight);
                return;
            }

            if (_animationRuntime == null)
            {
                LogAnimationContractWarning($"SetPlayableWeightAssumeBound: _animationRuntime 为空，降级为 SetPlayableWeight。State={GetStateNameSafe()}");
                SetPlayableWeight(weight);
                return;
            }
        ...
        internal void SetEffectivePlayableWeightNoDirty(float weight)
        {
            if (weight < 0f || weight > 1f)
            {
                LogAnimationContractWarning($"SetEffectivePlayableWeightNoDirty: weight={weight} 超出[0,1]，已自动Clamp。State={GetStateNameSafe()}");
                weight = Mathf.Clamp01(weight);
            }
            _playableWeight = weight;
            if (_animationRuntime != null)
        ...
        public bool CreatePlayable(PlayableGraph graph, out Playable output)
        {
            ...
            if (runtime != null && runtime.IsInitialized)
            {
                ...
                {
                    LogAnimationContractWarning(
                        $"CreatePlayable: runtime/calculator 类型不一致，已拒绝复用并返回失败。State={GetStateNameSafe()} Bound={runtime.BoundCalculatorKind} Current={_calculatorCached.CalculatorKind}");
                    output = Playable.Null;
                    return false;
                }
#endif
            ...
            if (runtime == null)
            {
                LogAnimationContractWarning($"CreatePlayable: runtime 创建失败，返回false。State={GetStateNameSafe()}");
                return false;
            }
#else
        ...
        private void CheckAnimationCompletion()
        {
            ...
            LogAnimationContractWarning(
                $"CheckAnimationCompletion: Calculator={calculator.GetType().Name} 未提供有效标准时长({standardDuration})，已跳过自动退出判定。State={GetStateNameSafe()}");
            return;
        }
        ...
        public bool ShouldAutoExit(float currentTime)
        {
            ...
            if (config == null)
            {
                LogAnimationContractWarning("ShouldAutoExit: basicConfig缓存为空，已降级为false。");
                return false;
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void LogAnimationContractWarning(string message)
        {
            var dbg = StateMachineDebugSettings.Instance;
            if (dbg != null)
            {
