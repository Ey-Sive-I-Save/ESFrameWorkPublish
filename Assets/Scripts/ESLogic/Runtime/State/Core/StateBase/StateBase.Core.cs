...
namespace ES
{
    ...
    public partial class StateBase : IPoolableAuto
    {
        ...
        /// <summary>
        /// 显式将 ResolvedRuntimeConfig 标记为失效。
        /// 当外部在运行时修改了 mergeData/costData/basicConfig 等会影响合成结果的配置时，
        /// 调用此方法可确保下一次访问 ResolvedConfig 时重新计算。
        /// </summary>
        public void MarkResolvedConfigDirty()
        {
            _resolvedRuntimeDirty = true;
        }

        /// <summary>
        /// 显式刷新：先重建 SharedData 缓存，再立即重算 ResolvedRuntimeConfig。
        /// 用于运行时配置热更新后“本帧立刻生效”的场景。
        /// </summary>
        public void RefreshSharedDataCacheAndResolvedConfig()
        {
            InternalRefreshSharedDataCache();
            _resolvedRuntimeDirty = true;
