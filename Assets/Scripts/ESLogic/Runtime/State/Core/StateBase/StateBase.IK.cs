...
namespace ES
{
    public partial class StateBase
    {
        ...
        private AnimationCalculatorRuntime EnsureIKRuntimeContainer()
        {
            if (_animationRuntime == null)
            {
                _animationRuntime = AnimationCalculatorRuntime.Pool.GetInPool();
        ...
        public void SetIKHintPosition(IKGoal goal, Vector3 position)
        {
            var runtime = EnsureIKRuntimeContainer();
            _ikActive = true;
            runtime.SetIKHintPosition(goal, position);
        }
        ...
        public void SetLookAtTarget(Vector3 position, float weight, float lerpingRate = 1f)
        {
            var runtime = EnsureIKRuntimeContainer();
            _ikActive = true;
            runtime.SetLookAtTarget(position, weight, lerpingRate);
        }
            ...
            float bodyWeight, float headWeight, float eyesWeight, float clampWeight)
        {
            var runtime = EnsureIKRuntimeContainer();
            _ikActive = true;
            runtime.SetLookAtTarget(position, weight, lerpingRate, bodyWeight, headWeight, eyesWeight, clampWeight);
        }
