...
//   每帧以 matchtargetTowardPosMultipler=3f 倍率和 positionApproachSpeed（单位/秒）进行：
...
namespace ES
{
    public partial class StateBase
    {
        ...
        [NonSerialized] private bool _runtimeIndependentTarget0Enabled;
        [NonSerialized] private Vector3 _runtimeIndependentTarget0Pos;
        [NonSerialized] private Quaternion _runtimeIndependentTarget0Rot = Quaternion.identity;

        [NonSerialized] private bool _runtimeIndependentTarget1Enabled;
        [NonSerialized] private Vector3 _runtimeIndependentTarget1Pos;
        [NonSerialized] private Quaternion _runtimeIndependentTarget1Rot = Quaternion.identity;

        [NonSerialized] private bool _runtimeIndependentTarget2Enabled;
        [NonSerialized] private Vector3 _runtimeIndependentTarget2Pos;
        [NonSerialized] private Quaternion _runtimeIndependentTarget2Rot = Quaternion.identity;
        ...
        private void ApplyMatchTargetConfigOnEnter()
        {
            ...
            ClearRuntimeIndependentTargets();
        ...
        /// <summary>
        /// 设置运行时独立目标槽位（索引仅支持 0/1/2）。
        /// 当时间线步骤勾选“使用独立目标点”时，会优先读取此处的运行时槽位。
        /// </summary>
        public void SetMatchTargetIndependentTarget(int index, Vector3 position, Quaternion rotation, bool enabled = true)
        {
            switch (index)
            {
                case 0:
                    _runtimeIndependentTarget0Enabled = enabled;
                    _runtimeIndependentTarget0Pos = position;
                    _runtimeIndependentTarget0Rot = rotation;
                    break;
                case 1:
                    _runtimeIndependentTarget1Enabled = enabled;
                    _runtimeIndependentTarget1Pos = position;
                    _runtimeIndependentTarget1Rot = rotation;
                    break;
                case 2:
                    _runtimeIndependentTarget2Enabled = enabled;
                    _runtimeIndependentTarget2Pos = position;
                    _runtimeIndependentTarget2Rot = rotation;
                    break;
                default:
#if UNITY_EDITOR
                    if (debugMatchTarget)
                        Debug.LogWarning($"{GetMatchTargetLogTag()} SetMatchTargetIndependentTarget: 非法索引={index}，仅支持 0/1/2");
#endif
                    break;
            }
        }
        ...
        /// <summary>
        /// 通过 Transform 设置运行时独立目标槽位（索引仅支持 0/1/2）。
        /// 传入 null 会自动禁用该槽位。
        /// </summary>
        public void SetMatchTargetIndependentTargetFromTransform(int index, Transform target)
        {
            if (target == null)
            {
                DisableMatchTargetIndependentTarget(index);
                return;
            }

            SetMatchTargetIndependentTarget(index, target.position, target.rotation, true);
        }

        /// <summary>
        /// 禁用运行时独立目标槽位（索引仅支持 0/1/2）。
        /// </summary>
        public void DisableMatchTargetIndependentTarget(int index)
        {
            switch (index)
            {
                case 0:
                    _runtimeIndependentTarget0Enabled = false;
                    break;
                case 1:
                    _runtimeIndependentTarget1Enabled = false;
                    break;
                case 2:
                    _runtimeIndependentTarget2Enabled = false;
                    break;
                default:
#if UNITY_EDITOR
                    if (debugMatchTarget)
                        Debug.LogWarning($"{GetMatchTargetLogTag()} DisableMatchTargetIndependentTarget: 非法索引={index}，仅支持 0/1/2");
#endif
                    break;
            }
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private bool TryGetRuntimeIndependentTargetPose(int index, out Vector3 pos, out Quaternion rot)
        {
            switch (index)
            {
                case 0 when _runtimeIndependentTarget0Enabled:
                    pos = _runtimeIndependentTarget0Pos;
                    rot = _runtimeIndependentTarget0Rot;
                    return true;
                case 1 when _runtimeIndependentTarget1Enabled:
                    pos = _runtimeIndependentTarget1Pos;
                    rot = _runtimeIndependentTarget1Rot;
                    return true;
                case 2 when _runtimeIndependentTarget2Enabled:
                    pos = _runtimeIndependentTarget2Pos;
                    rot = _runtimeIndependentTarget2Rot;
                    return true;
                default:
                    pos = Vector3.zero;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void ClearRuntimeIndependentTargets()
        {
            _runtimeIndependentTarget0Enabled = false;
            _runtimeIndependentTarget0Pos = Vector3.zero;
            _runtimeIndependentTarget0Rot = Quaternion.identity;

            _runtimeIndependentTarget1Enabled = false;
            _runtimeIndependentTarget1Pos = Vector3.zero;
            _runtimeIndependentTarget1Rot = Quaternion.identity;

            _runtimeIndependentTarget2Enabled = false;
            _runtimeIndependentTarget2Pos = Vector3.zero;
            _runtimeIndependentTarget2Rot = Quaternion.identity;
        }
        ...
        private void TickConfigAutoMatchTarget(float elapsed)
        {
            ...
            if (latestReadyStep == null || latestReadyStep.request == null)
                return;

            Vector3 timelineTargetPos = _sharedMatchTargetPos;
            Quaternion timelineTargetRot = _sharedMatchTargetRot;
            if (latestReadyStep.useIndependentTargetPoint)
            {
                // 独立目标槽位按“独立步骤出现顺序”决定：
                // 第1个独立步骤->Index0，第2个独立步骤->Index1，第3个独立步骤->Index2。
                // 这样即使前面有非独立步骤，也不会占用独立槽位序号。
                int independentStepOrderIndex = ResolveIndependentStepOrderIndex(timeline, latestReadyIndex);
                int resolvedIndependentIndex = Mathf.Clamp(independentStepOrderIndex, 0, 2);
#if UNITY_EDITOR
                if (debugMatchTarget && independentStepOrderIndex > 2)
                {
                    Debug.LogWarning(
                        $"{GetMatchTargetLogTag()} [Config-Timeline] 独立步骤序号={independentStepOrderIndex} 超出独立目标槽位上限(仅0/1/2)，已钳制到Index=2");
                }
#endif

                bool fromRuntime = TryGetRuntimeIndependentTargetPose(resolvedIndependentIndex, out var independentPos, out var independentRot);
                bool fromConfig = false;
                if (!fromRuntime)
                    fromConfig = proceduralDriveConfig.TryGetIndependentMatchTargetPose(resolvedIndependentIndex, out independentPos, out independentRot);

                if (fromRuntime || fromConfig)
                {
                    timelineTargetPos = independentPos;
                    timelineTargetRot = independentRot;
                }
#if UNITY_EDITOR
                else if (debugMatchTarget)
                {
                    Debug.LogWarning(
                        $"{GetMatchTargetLogTag()} [Config-Timeline] Step={latestReadyIndex} 请求独立目标索引={resolvedIndependentIndex}，但槽位未启用，回退共享目标位姿");
                }
#endif
                ...
                if (debugMatchTarget)
                {
                    string source = fromRuntime ? "runtime" : (fromConfig ? "config" : "shared-fallback");
                    Debug.Log($"{GetMatchTargetLogTag()} [Config-Timeline] Step={latestReadyIndex} independentOrder={independentStepOrderIndex} 独立目标来源={source}");
                }
#endif
            }

            ApplyMatchTargetRequest(latestReadyStep.request, timelineTargetPos, timelineTargetRot, "Config-Timeline");
#if UNITY_EDITOR
            if (debugMatchTarget)
                Debug.Log($"{GetMatchTargetLogTag()} [Config-Timeline] Step={latestReadyIndex} fired elapsed={elapsed:F3}s triggerAt={latestReadyStep.triggerAt:F3}s useIndependentTarget={latestReadyStep.useIndependentTargetPoint}");
#endif
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static int ResolveIndependentStepOrderIndex(System.Collections.Generic.List<MatchTargetPendingCommand> timeline, int stepIndex)
        {
            if (timeline == null || stepIndex < 0)
                return 0;

            int independentOrder = -1;
            int upper = Mathf.Min(stepIndex, timeline.Count - 1);
            for (int i = 0; i <= upper; i++)
            {
                var step = timeline[i];
                if (step != null && step.useIndependentTargetPoint)
                    independentOrder++;
            }

            return independentOrder < 0 ? 0 : independentOrder;
        }
        ...
        // matchtargetTowardPosMultipler / matchtargetTowardRotMultipler 已移除：
        ...
        internal void ProcessMatchTarget(Animator animator)
        {
            ...
            // 乘以速度倍率常量，控制整体逼近快慢
            const float matchtargetTowardPosMultipler = 3f;
            const float matchtargetTowardRotMultipler = 1f;
