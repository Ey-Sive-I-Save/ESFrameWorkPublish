...
namespace ES
{
    ...
    public class StateSharedData : IRuntimeInitializable
    {
        ...
        public bool RequiresStateMachinePlayableAnimation =>
            hasAnimation && (animationSource == StateAnimationSource.StateConfig || animationSource == StateAnimationSource.SkillTimeline);

        public bool UsesSkillTimelineAnimation =>
            hasAnimation && animationSource == StateAnimationSource.SkillTimeline;



        [HideLabel]
        [TabGroup("状态配置", "动画资源", Order = 2, TextColor = "@ESDesignUtility.ColorSelector.GetColor(\"雾绿\")"), PropertyOrder(1)]
        [ShowIf("RequiresStateMachinePlayableAnimation")]
        [InlineProperty]
        ...
        [TabGroup("状态配置", "动画资源", Order = 2, TextColor = "@ESDesignUtility.ColorSelector.GetColor(\"雾绿\")"), PropertyOrder(1)]
        [BoxGroup("状态配置/动画资源/过渡/高级设置", ShowLabel = true)]
        [ShowIf("@RequiresStateMachinePlayableAnimation && useAdvancedFadeCurve")]
        [EnableIf("enableFadeInOut")]
        [LabelText("淡入曲线")]
        [Tooltip("自定义淡入权重曲线")]
        ...
        [Tooltip("被覆盖时发出通知")]
        public bool notifyOnOverride = false;
        ...
        {
#if UNITY_EDITOR
            if (!EditorUtility.DisplayDialog("初始化确认", "将重新初始化运行数据，可能影响已配置内容。是否继续？", "继续", "取消"))
            {
                return;
            }
#endif
            ...
            InitializeRuntime();
            Debug.Log("[StateSharedData] 初始化完成");
        }
        ...
        {
#if UNITY_EDITOR
            if (!EditorUtility.DisplayDialog("应用预设确认", $"将应用预设：{preset}。这会覆盖当前配置内容。是否继续？", "继续", "取消"))
            {
                return;
            }
#endif
        ...
        public StateSharedData CloneWithAnimation(string newStateName, int newStateId, StateAnimationConfigData newAnimation)
        {
            ...
            cloned.animationSource = newAnimation != null ? StateAnimationSource.StateConfig : StateAnimationSource.None;
