...
namespace ES
{
    ...
    public class StateMergeData : IRuntimeInitializable
    {
        ...
        [HideLabel]
        public NormalMergeRule asLeftRule = new NormalMergeRule();

        [BoxGroup("对称条件/作为尝试加入者")]
        [Tooltip("作为尝试加入者：当新状态想加入时，本规则用于决定是否能打断已有状态或被拒绝。")]
        [HideLabel]
        public NormalMergeRule asRightRule = new NormalMergeRule();
        ...
        public void InitializeRuntime()
        {
            ...
            asLeftRule ??= new NormalMergeRule();
            asRightRule ??= new NormalMergeRule();
            asLeftRule.unconditionalRule ??= new List<UnconditionalMatchRule>();
            asRightRule.unconditionalRule ??= new List<UnconditionalMatchRule>();
