...
namespace ES
{
    ...
    public partial class StateMachine
    {
        ...
        // 其它检查器展示字段（如默认状态键、遮罩、参考姿态等）
        [TabGroup("SM_View", "⚙ 配置")]
        [BoxGroup("SM_View/⚙ 配置/基础设置", ShowLabel = false), LabelText("默认状态键"), ValueDropdown("GetAllStateKeys")]
        public string defaultStateKey;

        [TabGroup("SM_View", "⚙ 配置")]
        [BoxGroup("SM_View/⚙ 配置/层级遮罩", ShowLabel = false), LabelText("上半身遮罩"), AssetsOnly]
        public AvatarMask upperBodyMask;

        [BoxGroup("SM_View/⚙ 配置/层级遮罩", ShowLabel = false), LabelText("下半身遮罩"), AssetsOnly]
        ...
        // 构造函数（如需手动new，Unity序列化会自动调用无参构造）
        public StateMachine()
        {
            // 组分已在字段声明时初始化，确保零GC
        }
        ...
        /// <summary>
        /// IK 聚合诊断摘要：按运行态显示写入前权重、门控原因与有效贡献。
        /// </summary>
        [NonSerialized]
        ...
        private AvatarMask ResolveLayerMask(StateLayerType layerType)
        {
            ...
            {
                case StateLayerType.UpperBody:
                    return this.upperBodyMask != null
                        ? this.upperBodyMask
                        : globalConfig != null ? globalConfig.upperBodyMask : null;
                case StateLayerType.LowerBody:
                    return this.lowerBodyMask != null
                        ? this.lowerBodyMask
        ...
        public void Dispose()
        {
            ...
            if (baseLayer != null)
            {
                foreach (var state in baseLayer.runningStates)
                {
                    state.DestroyPlayable();
                }
                baseLayer.runningStates.Clear();
                baseLayer.stateToSlotMap.Clear();
                baseLayer.InternalClearConnectedStates();
                baseLayer.freeSlots.Clear();
                foreach (var kvp in baseLayer.fadeOutStates)
                {
                    if (kvp.Value != null)
                    {
                        kvp.Value.TryAutoPushedToPool();
                ...
                baseLayer.fadeOutStates.Clear();
                foreach (var kvp in baseLayer.fadeInStates)
                {
                    if (kvp.Value != null)
                    {
                        kvp.Value.TryAutoPushedToPool();
                ...
                baseLayer.fadeInStates.Clear();
                if (baseLayer.mixer.IsValid())
                    baseLayer.mixer.Destroy();
            }
            // Main Layer
            if (mainLayer != null)
            {
                foreach (var state in mainLayer.runningStates)
                {
                    state.DestroyPlayable();
                }
                mainLayer.runningStates.Clear();
                mainLayer.stateToSlotMap.Clear();
                mainLayer.InternalClearConnectedStates();
                mainLayer.freeSlots.Clear();
                foreach (var kvp in mainLayer.fadeOutStates)
                {
                    if (kvp.Value != null)
                    {
                        kvp.Value.TryAutoPushedToPool();
                ...
                mainLayer.fadeOutStates.Clear();
                foreach (var kvp in mainLayer.fadeInStates)
                {
                    if (kvp.Value != null)
                    {
                        kvp.Value.TryAutoPushedToPool();
                ...
                mainLayer.fadeInStates.Clear();
                if (mainLayer.mixer.IsValid())
                    mainLayer.mixer.Destroy();
            }
            // Buff Layer
            if (buffLayer != null)
            {
                foreach (var state in buffLayer.runningStates)
                {
                    state.DestroyPlayable();
                }
                buffLayer.runningStates.Clear();
                buffLayer.stateToSlotMap.Clear();
                buffLayer.InternalClearConnectedStates();
                buffLayer.freeSlots.Clear();
                foreach (var kvp in buffLayer.fadeOutStates)
                {
                    if (kvp.Value != null)
                    {
                        kvp.Value.TryAutoPushedToPool();
                ...
                buffLayer.fadeOutStates.Clear();
                foreach (var kvp in buffLayer.fadeInStates)
                {
                    if (kvp.Value != null)
                    {
                        kvp.Value.TryAutoPushedToPool();
                ...
                buffLayer.fadeInStates.Clear();
                if (buffLayer.mixer.IsValid())
                    buffLayer.mixer.Destroy();
            }
            // UpperBody Layer
            if (upperBodyLayer != null)
            {
                foreach (var state in upperBodyLayer.runningStates)
                {
                    state.DestroyPlayable();
                }
                upperBodyLayer.runningStates.Clear();
                upperBodyLayer.stateToSlotMap.Clear();
                upperBodyLayer.InternalClearConnectedStates();
                upperBodyLayer.freeSlots.Clear();
                foreach (var kvp in upperBodyLayer.fadeOutStates)
                {
                    if (kvp.Value != null)
                    {
                        kvp.Value.TryAutoPushedToPool();
                ...
                upperBodyLayer.fadeOutStates.Clear();
                foreach (var kvp in upperBodyLayer.fadeInStates)
                {
                    if (kvp.Value != null)
                    {
                        kvp.Value.TryAutoPushedToPool();
                ...
                upperBodyLayer.fadeInStates.Clear();
                if (upperBodyLayer.mixer.IsValid())
                    upperBodyLayer.mixer.Destroy();
            }
            // LowerBody Layer
            if (lowerBodyLayer != null)
            {
                foreach (var state in lowerBodyLayer.runningStates)
                {
                    state.DestroyPlayable();
                }
                lowerBodyLayer.runningStates.Clear();
                lowerBodyLayer.stateToSlotMap.Clear();
                lowerBodyLayer.InternalClearConnectedStates();
                lowerBodyLayer.freeSlots.Clear();
                foreach (var kvp in lowerBodyLayer.fadeOutStates)
                {
                    if (kvp.Value != null)
                    {
                        kvp.Value.TryAutoPushedToPool();
                ...
                lowerBodyLayer.fadeOutStates.Clear();
                foreach (var kvp in lowerBodyLayer.fadeInStates)
                {
                    if (kvp.Value != null)
                    {
                        kvp.Value.TryAutoPushedToPool();
                ...
                lowerBodyLayer.fadeInStates.Clear();
                if (lowerBodyLayer.mixer.IsValid())
                    lowerBodyLayer.mixer.Destroy();
            }
        ...
        public void UpdateStateMachine()
        {
            ...
            for (int i = 0; i < allRunningStates.Count; i++)
            {
                ...
                baseLayer.UpdateDirtyDecay();
                mainLayer.UpdateDirtyDecay();
                buffLayer.UpdateDirtyDecay();
                upperBodyLayer.UpdateDirtyDecay();
                lowerBodyLayer.UpdateDirtyDecay();

                // 根据Dirty等级处理不同任务（包括FallBack自动激活）
                ProcessDirtyTasks(baseLayer, StateLayerType.Base);
                ProcessDirtyTasks(mainLayer, StateLayerType.Main);
                ProcessDirtyTasks(buffLayer, StateLayerType.Buff);
                ProcessDirtyTasks(upperBodyLayer, StateLayerType.UpperBody);
                ProcessDirtyTasks(lowerBodyLayer, StateLayerType.LowerBody);
        ...
        private void UpdateStateGeneralFinalIKDriverPoseCache(float deltaTime)
        {
            ...
            for (int i = 0; i < allRunningStates.Count; i++)
            {
                ...
                if (!runtime.ik.HasAnyTargetWeight)
                {
                    ...
                    {
                        _ikContributionBuilder.Append("#").Append(i).Append(' ').Append(stateKey)
                            .Append(" Skip:all target weights <= 0")
                            .AppendLine();
                    }
                    continue;
                }

                // IK 作为动作校准层，不受 PlayableWeight 影响。
                float master = 1f;
        ...
        { 
            ...
            // 运行时代码直接写入（例如交互模块 SetIKGoal）时，不应被配置层开关硬清零。
                    ...
                    default:
                        {
                            ...
                            {
                                string existingStateKey = existingState != null ? existingState.strKey : "<null>";
                                dbg.LogWarning($"[TestStateActivation] Fail: Unexpected merge result with {existingStateKey}");
                            }
#endif
                            if (cache != null)
                            {
        ...
        /// </summary>
        /// 规则顺序：
        /// 1) 双向无条件规则（按名称/ID命中后直接返回）；
        /// 2) 通道掩码与命中方向（互斥/可打断/可共存）；
        /// 3) 同层停留等级与优先级判定（含 EqualIsEffectial）；
        /// 4) 可选代价预算校验（motion/agility/target）；
        /// 5) 无通道重叠时直接可并存。
            ...
            ref int totalMotionCost, ref int totalAgilityCost, ref int totalTargetCost)
        {
            ...
            if (existing == incoming)
            {
                ...
                if (dbg != null)
                {
                    dbg.LogWarning("[MergeCheck] Fail: existing == incoming");
                }
#endif
            ...
            NormalMergeRule leftRule = leftResolved.asLeftRule ?? (existingMergeData != null ? existingMergeData.asLeftRule : null);
            NormalMergeRule rightRule = rightResolved.asRightRule ?? (incomingMergeData != null ? incomingMergeData.asRightRule : null);
            ...
            if (channelOverlap)
            {
                ...
                if (onlyInterruptTest)
                {
                    ...
                    if (rightRule.hitByLayerOption == StateHitByLayerOption.Never)
                    {
                        ...
                        if (dbg != null)
                        {
                            dbg.LogWarning("[MergeCheck] Fail: Right hitByLayer=Never");
                        }
#endif
                        return StateMergeResult.MergeFail;
                    }
                    if (leftRule.hitByLayerOption == StateHitByLayerOption.Never)
                    {
                        ...
                        if (dbg != null)
                        {
                            dbg.LogWarning("[MergeCheck] Fail: Left hitByLayer=Never");
                        }
#endif
                        return StateMergeResult.MergeFail;
                    }
                    ...
                    if (levelOverlap == StateStayLevel.Rubbish)
                    {
                            ...
                            && rightRule.hitByLayerOption == StateHitByLayerOption.SameLevelTest)
                        {
                            ...
                            if (dbg != null)
                            {
                                dbg.LogWarning("[MergeCheck] Fail: SameLevelTest + Rubbish overlap");
                            }
#endif
                            return StateMergeResult.MergeFail;
                        }
                    ...
                    if (dbg != null)
                    {
                        dbg.LogWarning("[MergeCheck] Fail: Unable to decide interrupt direction");
                    }
#endif
                    return StateMergeResult.MergeFail;
                }
        ...
        /// </summary>
        /// 执行顺序：
        /// 1) 校验测试结果；2) 处理Restart与打断；3) 进入目标状态并加入运行集合；
        /// 4) 清理残留Fade并热插拔到Playable；5) 应用淡入并首帧立即更新动画；
        /// 6) 激活阶段异常时做回滚清理，避免集合/Playable污染。
        public bool ExecuteStateActivation(StateBase targetState, StateLayerType layer, in StateActivationResult result)
        {
            ...
            if ((result.code & StateActivationCode.Success) == 0)
            {
                PushActivationEvent(
                    ...
                    layer,
                    result.code,
                    ActivationEventKind.Failure,
                    ActivationFailureKind.Validation,
            ...
            if (layerRuntime == null)
            {
                PushActivationEvent(
                    ...
                    result.code,
                    ActivationEventKind.Failure,
                    ActivationFailureKind.LayerNotFound,
                    result.interruptCount,
                    runningBefore,
                    runningStates.Count,
                    0,
            ...
            PushActivationEvent(
                ...
                targetState,
                layer,
                layerRuntime.layerType,
                result.code,
                ActivationEventKind.Begin,
                ActivationFailureKind.None,
                result.interruptCount,
                runningBefore,
                runningStates.Count,
                layerRunningBefore,
            ...
            // 幂等保护：已运行且本次不是Restart时，不重复触发OnStateEnter，避免重复加入运行集合。
            ...
            if ((result.code & StateActivationCode.HasInterrupt) != 0)
            {
                ...
                if (interruptStates != null && result.interruptCount > 0)
                {
                    ...
                    for (int i = 0; i < interruptStates.Count; i++)
                    {
                        ...
                        TruelyDeactivateState(interruptStates[i], deactiveLayer);
                    }
                    ...
                    PushActivationEvent(
                        ...
                        layerRuntime.layerType,
                        result.code,
                        ActivationEventKind.Interrupt,
                        ActivationFailureKind.None,
                        result.interruptCount,
                        runningBefore,
                        runningStates.Count,
                        layerRunningBefore,
                        layerRuntime.runningStates != null ? layerRuntime.runningStates.Count : 0);
                }
            }
        ...
        private StateLayerRuntime ResolveConnectedLayerRuntimeForState(StateBase state, StateLayerType fallbackLayer, out StateLayerType resolvedLayer)
        {
            ...
            // Check Base Layer
            if (baseLayer != null &&
                (baseLayer.stateToSlotMap.ContainsKey(state)
                 || baseLayer.fadeInStates.ContainsKey(state)
                 || baseLayer.fadeOutStates.ContainsKey(state)
                 || baseLayer.runningStates.Contains(state)))
            {
            ...
            // Check Main Layer
            if (mainLayer != null &&
                (mainLayer.stateToSlotMap.ContainsKey(state)
                 || mainLayer.fadeInStates.ContainsKey(state)
                 || mainLayer.fadeOutStates.ContainsKey(state)
                 || mainLayer.runningStates.Contains(state)))
            {
            ...
            // Check Buff Layer
            if (buffLayer != null &&
                (buffLayer.stateToSlotMap.ContainsKey(state)
                 || buffLayer.fadeInStates.ContainsKey(state)
                 || buffLayer.fadeOutStates.ContainsKey(state)
                 || buffLayer.runningStates.Contains(state)))
            {
                resolvedLayer = StateLayerType.Buff;
                return buffLayer;
            }
            // Check UpperBody Layer
            if (upperBodyLayer != null &&
                (upperBodyLayer.stateToSlotMap.ContainsKey(state)
                 || upperBodyLayer.fadeInStates.ContainsKey(state)
                 || upperBodyLayer.fadeOutStates.ContainsKey(state)
                 || upperBodyLayer.runningStates.Contains(state)))
            {
                resolvedLayer = StateLayerType.UpperBody;
                return upperBodyLayer;
            }
            // Check LowerBody Layer
            if (lowerBodyLayer != null &&
                (lowerBodyLayer.stateToSlotMap.ContainsKey(state)
                 || lowerBodyLayer.fadeInStates.ContainsKey(state)
                 || lowerBodyLayer.fadeOutStates.ContainsKey(state)
                 || lowerBodyLayer.runningStates.Contains(state)))
            {
        ...
        /// <summary>
        /// 显式标记状态的 ResolvedConfig 失效。
        /// 适用于运行时修改 stateSharedData 的 merge/cost/basic 等配置后延迟重算场景。
        /// </summary>
        ...
        /// <summary>
        /// 显式刷新状态共享缓存并立即重算 ResolvedConfig。
        /// 适用于运行时热更新配置后要求“本帧立刻生效”的场景。
        /// </summary>
        ...
        /// <summary>
        /// 尝试获取Float参数 - 不存在时返回false，避免依赖魔法默认值。
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
