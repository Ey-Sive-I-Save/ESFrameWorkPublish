...
namespace SpriteShadersUltimate
{
    public class SSUShaderGUI : ShaderGUI
    {
        private static readonly Dictionary<string, string> ChineseLabels = new Dictionary<string, string>
        {
            ...
            { "UV Speed", "UV 流动速度" },
            { "Time Speed", "时间速度" },
            { "Shader Space", "Shader 坐标空间" },
            { "Fading Type", "淡化类型" },
            { "Noise Texture", "噪声纹理" },
            { "Pixel Snap", "像素对齐" },
            { "Tiling Fix", "平铺修正" },
            { "Sprite Sheet Fix", "序列帧修正" },
        };
        ...
        public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
        {
            ...
            for (int n = 0; n < properties.Length; n++)
            {
                ...
                {
                    DrawShaderCategory("all", ref showTime);
                }
                else if (prop.name == "_ShaderFading")
                {
                    ...
                    {
                        case (0):
                            DisplayHint("- 使用带尺寸修正的 <b>UV</b> 坐标。");
                            break;
                        case (1):
                            DisplayHint("- 使用原始 <b>UV</b> 坐标。");
                            break;
                        case (2):
                            DisplayHint("- 使用对象<b>局部坐标</b>。");
                            break;
                        case (3):
                            DisplayHint("- 使用带缩放修正的<b>局部坐标</b>。");
                            break;
                        case (4):
                            DisplayHint("- 使用<b>世界坐标</b>（适合场景效果）。");
                            break;
                        case (5):
                            DisplayHint("- 用于 <b>UI Image</b>。");
                            DisplayHint("- <b>ImageSSU</b> 组件会自动更新<b>矩形尺寸</b>。");
                            ...
                            break;
                        case (6):
                            DisplayHint("- 用于静止的 <b>UI Image</b> 或<b>全屏</b>效果。");
                            break;
                    }
                ...
                displayName = GetChineseLabel(displayName);
                ...
                {
                    DisplayHint("- all flows <b>back</b> and <b>forth</b>.");
                }
        ...
        void DrawShaderCategory(string title)
        {
            ...
            GUILayout.Label("<b><size=14>" + GetChineseLabel(title) + "</size></b>", labelStyle);
            Lines();
        }
        ...
        private static string GetChineseLabel(string label)
        {
            return ChineseLabels.TryGetValue(label, out string chineseLabel) ? chineseLabel : label;
        }
