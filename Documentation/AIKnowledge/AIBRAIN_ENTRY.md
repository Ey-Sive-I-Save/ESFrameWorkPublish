...
| 资源读取、投影与发布链 | `resource, reader, projection, json, table, html, bytes, asset, manifest, provider, resource-plan, owner-scope, temporary-scope, lease, provider-transition` | `es-resource-reader`, `es-resource-collection`, `es-resource-pipeline`, `es-resource-publish-audit` |
