...
`HashSchema`: `v2`
`ContentHash`: `fa7cac663483ee71f542de2c5c8f931ad4fadb38eed090628d704325929bb972`
`SourceSetHash`: `fa7cac663483ee71f542de2c5c8f931ad4fadb38eed090628d704325929bb972`
`EntryBodyHash`: `73183e3f121a14bfb2176215445b3b18243974a5da457f4aca0e32d62a9dc90d`
`EvidenceLevel`: `S0`
...
| 锚点/Canvas/父子布局推断 | 结构化设计 AST 优先；截图只能作候选 | Canvas owner、anchor/pivot、parent、axis owner、safe area、profile reflow | `Design IR`、约束求解器、UGUI LayoutPlan materializer | Canvas/RectTransform 规则与 Adapter 投影存在；严格 Validator 已拒绝 edge/center/stretch 几何矛盾、profile 内 sibling/layer 倒置、主动作色彩漂移及内容滥用 `safeArea: ignore`；高保真 `anchorContract` 还要求关键组件声明最终 Unity bottom-left `anchorMin/anchorMax` 并与 top-left bounds 投影一致，同时保留 authored Unity pivot；LayoutGroup 管理的子节点不得声明 authored final anchors；状态也强制复用基准 LayoutPlan | IR importer、通用 anchor solver、constraint graph、冲突诊断/修复 | 同轴多 owner；pivot 二次翻转；绝对像素复制；CanvasScaler 与安全区冲突；窄屏溢出 | 每 profile 的 resolved RectTransform、每状态 preserveBounds 回执、冲突列表为零或有明确降级、Unity 重读快照 |
...
| `UI-CAP-002` | 同轴同时由 anchor、LayoutGroup、ContentSizeFitter、固定尺寸驱动 | 每轴唯一 owner + constraint graph cycle check；现有快照已拒绝父 LayoutGroup 与子 ContentSizeFitter 同时控制同一轴 | 回退最后通过 Plan，重新求解受影响轴 | 通用跨树 solver、真实 Unity rebuild 负例回执 |
...
| `UI-CAP-006` | 静态通过、非空 PNG、不同 root/Canvas/viewport、RectTransform 层级/锚点或元素几何的 editor/UI 快照拼接，不可解析 parent、重复 sibling index、视口外 runtime rect、低于声明尺寸的 active Button，状态零差异、无关背景差异、正确区域改像素却未执行声明 effect，或旧 baseline 被当成视觉接受 | capture/source/profile/state identity + root/Canvas/viewport/path/active/parent resolution/sibling uniqueness/anchor/pivot/rect cross-check + active Button target-size check + pixel/structure rubric；有 visualChanges 的 non-default state 必须相对 default 产生最小像素差，且差异多数落在 default editor snapshot 的 declared affected-component 区域；每条 effect 还需在目标 UI snapshot 中核对 capability 和最终字段值 | 废弃该 evidence group，用新 runId 重采集 | automatic evaluator + repair ledger |
...
| `UI-CAP-012` | 控件 bounds、锚点与 target 尺寸全部合法，却被 RectMask2D 裁剪、CanvasGroup 禁用或同父不透明 overlay 抢走射线 | 配对快照强制序列化/比较 clipping chain、可见交集/fraction、CanvasGroup 链、reachability 和保守 blocker；active/interactable target 需满足最小可见尺寸；非矩形 Mask 不能伪装已证明 | 修订层级/Mask/CanvasGroup/overlay 或在 loading/disabled 状态禁用被覆盖目标，重采 profile/state 证据 | EventSystem 真正命中、非矩形 stencil 与嵌套 Canvas 排序仍需 Unity Runtime/PlayMode 输入回执 |
| `UI-CAP-013` | profile 组件数量或区域为空，却因组件树非空而误判为完整商业界面 | `UI-FB-010` 与 `contentRequirements` 强制每 profile 的组件/文本/交互下限、requiredTypes 和 zone minimum，并绑定确定性计数 | 补齐 ScreenSpec 的内容区域或明确降级为 layout-only | profile 视觉截图与人工内容完整性复核 |
| `UI-CAP-014` | 声明的颜色、状态 Token 或间距尺度没有消费者，状态只写名称没有可执行效果 | `UI-FB-011` 与 `requiredTokenConsumers`、`stateTokenBindings`、`spacingScalePx` 检查消费者计数、状态 effect 解析和尺度成员关系 | 为组件补充 Token/状态 effect，或移除未使用的声明后重跑全部 profile/state | Unity 材质/字体导入和 GPU 状态对比仍需运行证据 |
| `UI-CAP-015` | 组件树合法但视觉层级无覆盖或主动作不在 strongest action band | `UI-FB-012` 与 `visualHierarchy` 检查 band 唯一性、rank/emphasis 递增、必需组件覆盖和主动作归属 | 重做 hierarchy/component membership，不以 sibling order 或颜色名代替层级 | Unity/GPU 层级辨识和人工视觉复核 |
| `UI-CAP-016` | 焦点顺序依赖层级猜测，或 loading/disabled 主动作仍可重复触发 | `UI-FB-013` 与 `interactionContract` 检查逐 profile 完整 focus order、默认主动作以及两状态 `interactable: false` | 修订 focus graph 候选与状态 effects，重跑 profile/state 输入矩阵 | EventSystem/Input System 的真实焦点遍历与输入回执 |
| `UI-CAP-017` | 局部 selected/loading/error 状态无界影响整个 profile | `UI-FB-014` 与 `stateImpactPolicy` 逐 profile 计算 active affected-component ratio | 缩小 affected set 或显式升级为 screen-wide mode | Unity/GPU 的实际状态变化区域证据 |
| `UI-CAP-018` | 锚点合同只是方向词，数值与 Materializer 最终 RectTransform 不一致或 pivot 被二次翻转 | `UI-FB-015` 与 `anchorContract` 校验 top-left bounds 到 Unity bottom-left anchor 投影、pivot 保留和 LayoutGroup owner 边界 | 修订 LayoutPlan/Materializer，并重采 paired RectTransform snapshot | Unity Layout rebuild 后的真实 anchor/pivot/rect 快照 |

## Current ES verdict

- `Knowledge`: 已有七项相关 owner，新增本条目负责就绪判定。
- `Protocol`: 六层都有不同程度的 owner/字段约定；ScreenSpec、AssetManifest resolver、LayoutPlan resolver、Token/Typography evaluator 与 Fixture/Materializer 链已有静态实现。高保真合同已加入视觉 band、focus order、状态影响预算和关键 RectTransform anchor 投影；状态合同已具备 Adapter 无损传递、执行集限制、双向静态绑定、目标级 effect 合同和跨 Validator/Resolver/Materializer 的 `preserveBounds` 几何不变量；Design IR、通用 solver、业务 Bridge、自动视觉 evaluator 与 repair loop 尚未闭合为可执行合同。
...
- `Documentation/AIKnowledge/UI/game-ui-vision-input-source-snapshot.md` (`12a09d727b9f8e5443cfa88e9c3a369056fcf182dd45f597992384c9b6055e13`)
...
- `Documentation/AIKnowledge/entries/game-ui-open-source-automation-patterns.md` (`6c8a48f8c928652651e52509983a6eb9753320934f272796ea64550044cf78d4`)
- `Documentation/AIKnowledge/entries/game-ui-visual-design-system.md` (`e6b1ede92447bee5d7d866eb640e3185ae4cdb2848b6e43936282b4bd203eda1`)
