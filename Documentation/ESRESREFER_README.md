...
|------|------|
| `ESResRefer` | 通用对象 |
| `ESResReferPrefab` | GameObject预制体 |
...
ESResRefer不具有独立的引用计数权限，所有引用计数由ES资源系统管理：

```csharp
// ESResRefer只是辅助工具，实际资源管理在ES系统内部
// - ESResSource管理资源状态
// - ESResTable管理引用计数
// - ESResMaster调度加载任务
```
...
在Inspector中，ESResRefer字段会显示 `@` 符号前缀，表明这是ES资源系统的便捷引用工具。
...
// ESResRefer在不传入Loader时会自动使用全局Loader
