...
适用源码入口：`Assets/Scripts/ESLogic/Runtime/Camera`、`Assets/Scripts/ESLogic/Editor/Camera`、`ESGameManager.Camera`、`EntityCharacterProfile`、`EntityAIInputDispatchModule`。
...
- `EntityCharacterProfile` 只提供默认 Base 的内容意图；只有 `ESGameManager.LocalControl` 当前控制的 Entity 才能实际申请它。普通 AI/NPC 即使填了 ProfileKey 也不能进入 `MainView` 仲裁。
...
Modifier 不抢占 Base/Shot。它只对当前赢家且 `compatibleProfileKey` 匹配时生效。
...
Profile 提供字段基线；最终值为 `((Override 或 Base) + Add) * Multiply`。这不是依赖调用顺序的隐式覆盖。

## 内容与场景

- `ESCameraProfile`：`ProfileKey -> RigKey + 输入/镜头基线`。
- `ESCameraRigCatalog`：`RigKey -> Rig Prefab`；禁止保存场景实例。
- `ESCameraProfileCatalog`：`ProfileKey -> Profile`。
...
- Runtime 的 `ICameraTrackPreviewFactory` 只包含接口、请求描述与稳定 `ProfileKey`；不引用 `UnityEditor`、`ESTrackViewWindow` 或 Cinemachine Editor 类型。
...
- 同一 Camera Track 的预览片段必须解析到同一对 `ProfileCatalog/RigCatalog`；有重复 Key 或跨 Catalog 混用时拒绝猜测，显式提示拆轨或统一内容目录。
...
- `ESCameraTimelineShot` 只保存 `ViewId`、`ProfileKey`、优先级、Owner、Follow 与 LookAt；它转换为普通 `Shot` 进入相同的 Director 活跃集合，不拥有旁路优先权。
