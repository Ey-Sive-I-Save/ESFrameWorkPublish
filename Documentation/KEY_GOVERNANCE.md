...
- `ESTagCollection` is the generic runtime container. It keeps aggregate HotSlot counts and sparse `RuntimeKey -> Count` entries only. Lease objects own their source reference and release exactly one increment; the collection does not retain source objects. `GetDebugSnapshot()` is diagnostic-only and stays off the hot path. `Entity.Tags` is one Host of this generic container.
...
- `ESTagGrantConfig` plus `ESTagLeaseSet` is the standard writer boundary for Buffs, equipped weapons, StateSupport projections, trigger zones, tasks, and future equipment/area systems. A writer acquires when its own lifecycle becomes active and disposes only its own LeaseSet on switch, exit, disable, or destroy. Direct count mutation and `RemoveAll` are not business writer APIs.
...
- `ESSuperAttributeCatalog` compiles value type, storage policy, defaults/range/formula/migration schema from generic attribute definitions. Character hot attributes now declare stable enum aliases separately from their HotSlot indices; optional modifiers use sparse `RuntimeKey` maps.
