...
function Test-ESABCEngineeringDimensionEvidence {
 [CmdletBinding()]param([Parameter(Mandatory)]$Architecture)
 $dims=@('lifecycle','performance','network','debugging','editorProduction');$rows=@();if($Architecture.PSObject.Properties['dimensionEvidence']){$rows=@($Architecture.dimensionEvidence)};$missing=@();foreach($dim in $dims){$r=@($rows|Where-Object{[string]$_.dimension -ceq $dim});if($r.Count -ne 1){$missing+="$dim.entry";continue};foreach($f in @('metric','target','observed','verificationRef')){if([string]::IsNullOrWhiteSpace([string]$r[0].$f)){$missing+="$dim.$f"}}};[pscustomobject][ordered]@{status=if($missing.Count -eq 0){'passed'}else{'failed'};dimensionCount=$dims.Count;missing=@($missing);failureCode='ENGINEERING_DIMENSION_EVIDENCE_REQUIRED'}
}
...
function Invoke-ESABCEngineeringArchitectureCompetition {
 ...
 if(-not [string]::IsNullOrWhiteSpace($ProjectRoot)){
  ...
  $requiredAssessments=@('implementationBoundary','lifecycleAssessment','performanceAssessment','networkAssessment','debuggingAssessment','editorProductionAssessment','playerIncrementRegression');$requiredScores=@('technicalInnovationLevel','safetyLevel','baselinePlayerLevel','playerIncrementDelta','performanceLevel','reusabilityLevel','lifecycleLevel','networkLevel','debuggingLevel','editorProductionLevel');$missingAssessment=@($requiredAssessments|Where-Object{[string]$_ -notin @($contract.requiredAssessments)});$missingScore=@($requiredScores|Where-Object{[string]$_ -notin @($contract.requiredScores)});$bands=@{'0'=-20;'1'=0;'2'=20;'3'=40;'4'=70;'5'=90};$badBand=@($bands.Keys|Where-Object{[double]$contract.scoreBands.$_ -ne [double]$bands[$_]})
  if([int]$contract.minimumGenerations -ne $MinimumGenerations -or [int]$contract.minimumDirections -gt $MinimumDirections -or $contract.completionRequiresImplementationEvidence -ne $true -or $contract.counterfactualGainRequired -ne $true -or $missingAssessment.Count -gt 0 -or $missingScore.Count -gt 0 -or $badBand.Count -gt 0){throw 'ENGINEERING_ARCHITECTURE_CONTRACT_MISMATCH'}
 }
 $allDirections=@();$generationHistory=@();$previous=@();$previousSignature='';$previousWinnerBand=$null;$previousWinner=$null;$independentReplayPassed=0
 for($generation=1;$generation -le $MinimumGenerations;$generation++){
  $response=@(& $ModelInvoker ([pscustomobject][ordered]@{phase='engineering-architecture-divergence';generationMode='engineering';round=$generation;requirement=$Requirement;goalRevision=$GoalRevision;sourceHash=$SourceHash;minimumDirections=$MinimumDirections;previousDirections=@($previous);requiredFields=@('implementationBoundary','lifecycleAssessment','performanceAssessment','networkAssessment','debuggingAssessment','editorProductionAssessment','playerIncrementRegression','innovationDifferentialAssessment','counterfactualRegression','innovationDifferentialLevel','technicalInnovationLevel','safetyLevel')}))
 ...
 $providerStability='unverified';if(@($ProviderScoreResults).Count -ge 2){try{$providerStability=(Compare-ESABCProviderEngineeringScores -EvaluationResults @($ProviderScoreResults)).status}catch{$providerStability='drift-failed'}};$implComplete=(@($allDirections|Where-Object{$_.status -eq 'eligible' -and $_.implementationEvidenceStatus -eq 'passed'}).Count -eq $eligible.Count);$replayComplete=($null -ne $ReplayInvoker -and $independentReplayPassed -eq $eligible.Count);$providerComplete=($providerStability -eq 'stable');$implComplete=($implComplete -and $replayComplete -and $providerComplete);$evidenceSummary=[ordered]@{eligibleCount=$eligible.Count;implementationEvidenceVerifiedCount=@($allDirections|Where-Object{$_.implementationEvidenceStatus -eq 'passed'}).Count;independentReplayVerifiedCount=$independentReplayPassed;providerScoreComparisonStatus=$providerStability;counterfactualPositiveCount=@($allDirections|Where-Object{$_.counterfactualGain -gt 0}).Count;generationImprovedCount=@($generationHistory|Where-Object{$_.incrementalGainStatus -eq 'improved'}).Count;generationNoIncrementCount=@($generationHistory|Where-Object{$_.incrementalGainStatus -eq 'no-engineering-increment'}).Count;contractHashBound=([string]$contractHash -match '^[0-9a-f]{64}$')}
...
function Invoke-ESABCEngineeringVerificationCommand {
 ...
 $job=Start-Job -ScriptBlock $Command;$completed=Wait-Job -Job $job -Timeout $TimeoutSeconds;$timedOut=($null -eq $completed);$exitCode=0;$output=''
 ...
 Remove-Job -Job $job -ErrorAction SilentlyContinue;$hash=([Security.Cryptography.SHA256]::Create().ComputeHash([Text.Encoding]::UTF8.GetBytes($output))|ForEach-Object ToString x2)-join ''
 [pscustomobject][ordered]@{status=if($exitCode -eq 0){'passed'}else{'failed'};exitCode=$exitCode;timedOut=$timedOut;output=$output;outputHash=$hash}
}
...
function Invoke-ESABCRequiredStageReview {
 ...
 try { $result=@(& $ModelInvoker ([pscustomobject][ordered]@{phase=$Stage;generationMode=$Run.generationMode;round=$i;runId=$Run.runId;parent=$Branch;weights=$Run.branchWeights;requirement=$Requirement;goalRevision=$GoalRevision;capabilityRefs=@($Run.capabilityPlan.capabilityRefs)})) }
...
function Invoke-ESABCInnovationRun {
 ...
 if($GenerationMode -eq 'engineering'){
  $architectureCompetition=Invoke-ESABCEngineeringArchitectureCompetition -Requirement $Requirement -GoalRevision $GoalRevision -SourceHash $SourceHash -ModelInvoker $ModelInvoker -ProjectRoot $ProjectRoot
