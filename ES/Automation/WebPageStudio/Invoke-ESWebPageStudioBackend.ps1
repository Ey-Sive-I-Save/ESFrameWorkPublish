...
if(-not $ExecuteNetwork){
  $baseForReceipt=[Uri]$contract.network.apiBase
  $receipt=[ordered]@{schemaVersion=1;recordType='WebPageStudioBackendRuntimeReceipt';receiptId=('network-not-run-'+[guid]::NewGuid().ToString('N'));contractId=$contract.contractId;status='not-run';method=$Method;targetHost=$baseForReceipt.Host;allowlist=[ordered]@{decision='allowed';matchedHost=$baseForReceipt.Host};timeoutSeconds=[int]$contract.network.timeoutSeconds;retry=[ordered]@{maxAttempts=[int]$contract.retry.maxAttempts;retryable=[bool]$contract.retry.enabled;exhausted=$false};recovery=[ordered]@{action='none';outcome='not-run'};attempts=0;networkCalls=0;httpStatus=0;error='';cancellation=[ordered]@{requested=$false;observed=$false};redaction=[ordered]@{applied=$true;responseBodyPersisted=$false;sensitiveHeadersPersisted=$false};runtimeStatus='runtime-not-run';nonClaims=@('ExecuteNetwork was not provided; no request was sent.','Response body and sensitive headers are not persisted.')}
...
$record=[ordered]@{schemaVersion=1;recordType='WebPageStudioBackendRuntimeReceipt';receiptId=('network-'+[guid]::NewGuid().ToString('N'));status=$status;contractId=$contract.contractId;method=$Method;targetHost=$target.Host;allowlist=[ordered]@{decision='allowed';matchedHost=$target.Host};timeoutSeconds=[int]$contract.network.timeoutSeconds;retry=[ordered]@{maxAttempts=[int]$contract.retry.maxAttempts;retryable=[bool]$contract.retry.enabled;exhausted=($null -eq $response -and $attempt -ge $max)};recovery=[ordered]@{action=if($null -eq $response){'manual'}else{'none'};outcome=if($null -eq $response){'failed'}else{'not-needed'}};attempts=$attempt;networkCalls=if($null -ne $response){1}else{0};httpStatus=if($null -ne $response){[int]$response.StatusCode}else{0};error=if($status -eq 'failed'){'[REDACTED]'}else{''};cancellation=[ordered]@{requested=$false;observed=$false};redaction=[ordered]@{applied=$true;responseBodyPersisted=$false;sensitiveHeadersPersisted=$false};runtimeStatus=if($status -eq 'passed'){'runtime-passed'}else{'runtime-failed'};nonClaims=@('Response body and sensitive headers are not persisted.','This receipt does not prove production availability or security posture.')}
