...
$required = @('index.html','web-page-contract.json','contract-validation.json','static-signals.json','design-tokens.json','deep-design.json','site.webmanifest','robots.txt','sitemap.xml','icon.svg')
...
Add-Check 'network-disabled' (($html -notmatch '(?is)<script\b') -and ($html -notmatch '(?is)(?:src|href|action)=["'']https?://') -and ($html -match '(?is)STATIC\s*/\s*NO NETWORK')) 'static HTML has no executable script or external resource URL'
