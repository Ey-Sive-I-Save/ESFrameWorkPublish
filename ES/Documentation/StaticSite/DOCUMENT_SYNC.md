...
- 当前门禁状态：`待分诊`。基线捕获后检测到源码快照变化；开放台账已捕获当前快照，但还没有拆分为可回归的完成项，因此不能通过重写指纹、更新台账哈希或提前改 HTML 绕过。
...
3. 以“已审阅 Git HEAD”为起点检查 `git log`、`git diff HEAD`、`git diff --cached` 与未跟踪源码文件。不要只依据提交信息，也不要把未完成的暂存内容当作已发布行为。
